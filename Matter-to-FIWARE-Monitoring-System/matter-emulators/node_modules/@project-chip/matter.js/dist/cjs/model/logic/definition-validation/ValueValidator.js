...
class ValueValidator extends import_ModelValidator.ModelValidator {
  validate() {
    ...
    this.validateProperty({ name: "metatype", type: import_definitions.Metatype });
  ...
  validateType() {
    ...
    if (metatype === import_definitions.Metatype.string && defaultValue === "empty") {
      defaultValue = "";
    }
    const cast = import_definitions.Metatype.cast(metatype, defaultValue);
    if (cast === import_definitions.FieldValue.Invalid) {
      this.error("INVALID_VALUE", `Value "${defaultValue}" is not a ${metatype}`);
      return;
    }
    defaultValue = cast;
    if (metatype === import_definitions.Metatype.enum) {
  ...
  validateEntries() {
    ...
    switch (metatype) {
      case import_definitions.Metatype.object:
        if (this.model.metatype || !this.model.children.length) {
          this.error("CHILDLESS_STRUCT", `struct element with no children`);
        }
        break;
      case import_definitions.Metatype.enum:
      case import_definitions.Metatype.bitmap:
        ...
        break;
      case import_definitions.Metatype.array:
  ...
  validateSpecialDefault(metatype, def) {
    if (typeof def === "object" && import_definitions.FieldValue.is(def, import_definitions.FieldValue.reference)) {
    ...
    if (typeof def === "string") {
      ...
      if (other) {
        this.model.default = import_definitions.FieldValue.Reference(other.name);
        return true;
      }
    }
    if (metatype === import_definitions.Metatype.bitmap && Array.isArray(def)) {
