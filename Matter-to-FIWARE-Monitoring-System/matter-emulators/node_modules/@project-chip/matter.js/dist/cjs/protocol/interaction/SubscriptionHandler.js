...
class SubscriptionHandler {
  constructor(subscriptionId, session, endpointStructure, attributeRequests, dataVersionFilters, eventRequests, eventFilters, eventHandler, isFabricFiltered, minIntervalFloor, maxIntervalCeiling, subscriptionMaxIntervalSeconds, subscriptionMinIntervalSeconds, subscriptionRandomizationWindowSeconds, cancelCallback) {
    ...
    this.updateTimer = import_Time.Time.getTimer(this.sendInterval, () => this.prepareDataUpdate());
    this.sendDelayTimer = import_Time.Time.getTimer(50, () => this.sendUpdate());
  }
  ...
  activateSendingUpdates() {
    ...
    this.updateTimer = import_Time.Time.getTimer(this.sendInterval, () => this.prepareDataUpdate()).start();
  }
  /**
  ...
  prepareDataUpdate() {
    ...
    if (timeSinceLastUpdateMs < this.minIntervalFloorMs) {
      this.updateTimer = import_Time.Time.getTimer(
  ...
  async sendInitialReport(messenger) {
    ...
    const dataVersionFilterMap = new Map(
      this.dataVersionFilters?.map(({ path, dataVersion }) => [(0, import_InteractionServer.clusterPathToId)(path), dataVersion]) ?? []
    );
  ...
  async sendUpdateMessage(attributes, events) {
    ...
    const exchange = this.server.initiateExchange(this.fabric, this.peerNodeId, import_InteractionServer.INTERACTION_PROTOCOL_ID);
    if (exchange === void 0)
      return;
    logger.debug(
      `Sending subscription changes for ID ${this.subscriptionId}: ${attributes.map(
        ({ path, value, version }) => `${this.endpointStructure.resolveAttributeName(path)}=${import_Logger.Logger.toJSON(value)} (${version})`
      ).join(", ")}`
    );
    ...
    try {
      await (0, import_TryCatchHandler.tryCatchAsync)(
        ...
        import_StatusCode.StatusResponseError,
        async (error) => {
          if (error.code === import_StatusCode.StatusCode.InvalidSubscription || error.code === import_StatusCode.StatusCode.Failure) {
