...
var MergeModels_exports = {};
__export(MergeModels_exports, {
  MergeModels: () => MergeModels
});
module.exports = __toCommonJS(MergeModels_exports);
 ...
 */
function MergeModels(variants, priorities = MergeModels.DefaultPriorities) {
  const priority = new PriorityHandler(priorities || MergeModels.DefaultPriorities);
  const visitor = new MergeTraversal(priority, (variants2, recurse) => {
    const merged = merge(variants2);
    if (variants2.tag === import_definitions.ElementTag.Cluster) {
      reparentToCanonicalParent(priority, variants2);
    }
  ...
  function merge(variants2) {
    ...
    const constraint = new import__.Constraint(properties.constraint);
    if (constraint.desc) {
      for (const key of priority.get("*", "constraint")) {
        const definition = variants2.map[key]?.constraint;
        const constraint2 = new import__.Constraint(definition);
...
class MergeTraversal extends import_ModelVariantTraversal.ModelVariantTraversal {
  ...
  chooseType(variants) {
    ...
    for (const sourceName of variantPriorities) {
      ...
      } else if (metatype === import_definitions.Metatype.integer) {
        const variantMetatype2 = variant.effectiveMetatype;
        if (variantMetatype2 === import_definitions.Metatype.enum || variantMetatype2 === import_definitions.Metatype.bitmap) {
          overridePriority = true;
        }
      }
      if (overridePriority) {
...
class PriorityHandler {
  ...
  get(typeName, fieldName) {
    ...
    if (!priority) {
      throw new import_MatterError.InternalError("No default (*, *) priority available");
    }
    return priority;
  }
}
function reparentToCanonicalParent(priority, variants) {
  const deparented = Array();
  const traversal = new MergeTraversal(priority, (variants2, recurse) => {
    const type = traversal.chooseType(variants2);
    if (!(type instanceof import_models.ValueModel)) {
      recurse();
      return;
    }
    if (type.base?.global && import_definitions.Metatype.hasChildren(type.effectiveMetatype)) {
      for (const variantName in variants2.map) {
        ...
        if (!(base instanceof import_models.ValueModel) || base.parent?.tag !== import_definitions.ElementTag.Cluster || !base.children.length) {
          continue;
        }
...
((MergeModels2) => {
  MergeModels2.DefaultPriorities = {
    "*": {
      "*": ["local", "chip", "spec"],
