...
class CaseServerMessenger extends import_SecureChannelMessenger.SecureChannelMessenger {
  async readSigma1() {
    const { payload } = await this.nextMessage(import_SecureChannelMessages.MessageType.Sigma1);
    return { sigma1Bytes: payload, sigma1: import_CaseMessages.TlvCaseSigma1.decode(payload) };
  }
  sendSigma2(sigma2) {
    return this.send(sigma2, import_SecureChannelMessages.MessageType.Sigma2, import_CaseMessages.TlvCaseSigma2);
  }
  sendSigma2Resume(sigma2Resume) {
    return this.send(sigma2Resume, import_SecureChannelMessages.MessageType.Sigma2Resume, import_CaseMessages.TlvCaseSigma2Resume);
  }
  async readSigma3() {
    const { payload } = await this.nextMessage(import_SecureChannelMessages.MessageType.Sigma3);
...
class CaseClientMessenger extends import_SecureChannelMessenger.SecureChannelMessenger {
  sendSigma1(sigma1) {
    return this.send(sigma1, import_SecureChannelMessages.MessageType.Sigma1, import_CaseMessages.TlvCaseSigma1);
  }
  async readSigma2() {
    const {
      payload,
    ...
    switch (messageType) {
      case import_SecureChannelMessages.MessageType.Sigma2:
        return { sigma2Bytes: payload, sigma2: import_CaseMessages.TlvCaseSigma2.decode(payload) };
      case import_SecureChannelMessages.MessageType.Sigma2Resume:
        return { sigma2Resume: import_CaseMessages.TlvCaseSigma2Resume.decode(payload) };
      default:
        throw new import_MatterError.MatterFlowError(
          `Received unexpected message type: ${messageType}, expected: ${import_SecureChannelMessages.MessageType.Sigma2} or ${import_SecureChannelMessages.MessageType.Sigma2Resume}`
        );
    }
  }
  sendSigma3(sigma3) {
    return this.send(sigma3, import_SecureChannelMessages.MessageType.Sigma3, import_CaseMessages.TlvCaseSigma3);
  }
}
