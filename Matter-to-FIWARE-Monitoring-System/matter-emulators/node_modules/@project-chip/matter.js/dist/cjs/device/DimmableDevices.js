...
var import_LevelControlCluster = require("../cluster/definitions/LevelControlCluster.js");
...
class DimmableBaseDevice extends (0, import_NamedHandler.extendPublicHandlerMethods)(
  ...
  addDeviceClusters(attributeInitialValues, excludeList = []) {
    super.addDeviceClusters(attributeInitialValues, [...excludeList, import_OnOffCluster.OnOff.Cluster.id, import_LevelControlCluster.LevelControl.Cluster.id]);
      ...
      this.addClusterServer(
        (0, import_OnOffServer.createDefaultOnOffClusterServer)(
          ...
          (0, import_OnOffDevices.getClusterInitialAttributeValues)(
            ...
            import_OnOffCluster.OnOff.Cluster.with(import_OnOffCluster.OnOff.Feature.LevelControlForLighting)
          ),
          this.isLighting
        )
      );
    }
    if (!excludeList.includes(import_LevelControlCluster.LevelControl.Cluster.id)) {
      this.addClusterServer(
        (0, import_LevelControlServer.createDefaultLevelControlClusterServer)(
          this.commandHandler,
          (0, import_OnOffDevices.getClusterInitialAttributeValues)(
            attributeInitialValues,
            import_LevelControlCluster.LevelControl.Cluster.with(import_LevelControlCluster.LevelControl.Feature.OnOff, import_LevelControlCluster.LevelControl.Feature.Lighting)
          )
        )
      );
    }
  ...
  getCurrentLevel() {
    return this.getClusterServer(import_LevelControlCluster.LevelControl.Cluster)?.getCurrentLevelAttribute() ?? 0;
  }
  setCurrentLevel(level) {
    this.getClusterServer(import_LevelControlCluster.LevelControl.Cluster)?.setCurrentLevelAttribute(level);
  }
  /**
  ...
  addCurrentLevelListener(listener) {
    this.getClusterServer(import_LevelControlCluster.LevelControl.Cluster)?.subscribeCurrentLevelAttribute(listener);
  }
}
class DimmablePluginUnitDevice extends DimmableBaseDevice {
  constructor(onOffAttributeInitialValues, dimmableAttributeValues, options = {}) {
    ...
    if (dimmableAttributeValues !== void 0) {
      initialAttributeValues[import_LevelControlCluster.LevelControl.Cluster.id] = dimmableAttributeValues;
    }
