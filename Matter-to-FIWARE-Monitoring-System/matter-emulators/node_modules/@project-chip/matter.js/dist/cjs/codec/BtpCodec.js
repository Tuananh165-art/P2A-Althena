...
var BtpCodec_exports = {};
__export(BtpCodec_exports, {
  BtpCodec: () => BtpCodec,
  BtpHeaderBits: () => BtpHeaderBits,
  BtpOpcode: () => BtpOpcode
});
module.exports = __toCommonJS(BtpCodec_exports);
var import_Ble = require("../ble/Ble.js");
var import_BtpSessionHandler = require("../ble/BtpSessionHandler.js");
...
class BtpCodec {
  static decodeBtpHandshakeRequest(data) {
    const reader = new import_DataReader.DataReader(data, import_ByteArray.Endian.Little);
  ...
  static encodeBtpPacket({ header, payload }) {
    return import_ByteArray.ByteArray.concat(this.encodeBtpPacketHeader(header), this.encodeBtpPacketPayload(header, payload));
  }
  ...
  static encodeBtpPacketPayload({ hasAckNumber, isBeginningSegment, isContinuingSegment, isEndingSegment }, { ackNumber, sequenceNumber, messageLength, segmentPayload }) {
    ...
    if (!hasAckNumber && ackNumber !== void 0) {
      throw new import_BtpSessionHandler.BtpProtocolError("Ack number shouldn't be set because header flag is not set.");
    }
    if (hasAckNumber && ackNumber === void 0) {
      throw new import_BtpSessionHandler.BtpProtocolError("Ack number needs to be set because header flag is set.");
    }
    if (isBeginningSegment && isContinuingSegment) {
      throw new import_BtpSessionHandler.BtpProtocolError("Beginning and continuing segment flags can't be set at the same time.");
    }
    if (isEndingSegment && !isContinuingSegment && !isBeginningSegment) {
      throw new import_BtpSessionHandler.BtpProtocolError("Ending segment flag can't be set without continuing segment flag.");
    }
    if ((isBeginningSegment || isContinuingSegment) && (segmentPayload === void 0 || segmentPayload.length === 0)) {
      throw new import_BtpSessionHandler.BtpProtocolError("Payload needs to be set because header flag indicates a message with payload.");
    }
    if (isBeginningSegment && messageLength === void 0) {
      throw new import_BtpSessionHandler.BtpProtocolError("Message length needs to be set because packet is a beginning segment.");
    }
    if (!isBeginningSegment && messageLength !== void 0) {
      throw new import_BtpSessionHandler.BtpProtocolError(
  ...
  static decodeHandshakeRequestPayload(reader) {
    ...
    if (header !== HANDSHAKE_HEADER) {
      throw new import_BtpSessionHandler.BtpProtocolError("Header for expected BTP Handshake Request is incorrect.");
    }
    if (opcode !== 108 /* HandshakeManagementOpcode */) {
      throw new import_BtpSessionHandler.BtpProtocolError("Management Opcode for BTP Handshake Request is incorrect.");
    }
    const ver = [];
    ver[0] = (version & 240) >> 4;
    ver[1] = version & 15;
    ...
    if (versions.length === 0) {
      throw new import_BtpSessionHandler.BtpProtocolError("No valid version provided.");
    }
  ...
  static decodeBtpPacketHeader(reader) {
    ...
    if (hasManagementOpcode) {
      throw new import_BtpSessionHandler.BtpProtocolError("Management Opcode for BTPHandshake Request is not expected");
    }
    return {
      isHandshakeRequest,
      hasManagementOpcode,
  ...
  static encodeBtpPacketHeader({
    ...
    if (isHandshakeRequest || hasManagementOpcode) {
      throw new import_BtpSessionHandler.BtpProtocolError("Please use the specific methods to encode a Handshake packet");
    }
    const header = (
  ...
  static decodeBleAdvertisementData(data) {
    ...
    if (reader.readUInt8() !== 2 || reader.readUInt8() !== 1 || reader.readUInt8() !== 6 || reader.readUInt8() !== 11 || reader.readUInt8() !== 22) {
      throw new import_Ble.BleError("Invalid BLE advertisement data");
    }
