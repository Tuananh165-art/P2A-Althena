...
class TlvNumericSchema extends import_TlvSchema.TlvSchema {
  ...
  decodeTlvInternalValue(reader, typeLength) {
    if (typeLength.type !== this.type)
      throw new import_MatterError.UnexpectedDataError(`Unexpected type ${typeLength.type}, was expecting ${this.type}.`);
    return reader.readPrimitive(typeLength);
  }
  validate(value) {
    if (typeof value !== "number" && typeof value !== "bigint")
      throw new import_ValidationError.ValidationError(`Expected number, got ${typeof value}.`);
    this.validateBoundaries(value);
  }
  validateBoundaries(value) {
    if (this.min !== void 0 && value < this.min)
      throw new import_ValidationError.ValidationError(`Invalid value: ${value} is below the minimum, ${this.min}.`);
    if (this.max !== void 0 && value > this.max)
      throw new import_ValidationError.ValidationError(`Invalid value: ${value} is above the maximum, ${this.max}.`);
  }
  ...
  bound({ min, max }) {
    return new TlvNumericSchema(
      this.type,
      this.lengthProvider,
      (0, import_Number.maxValue)(min, this.min),
      (0, import_Number.minValue)(max, this.max)
    );
  }
}
class TlvNumberSchema extends TlvNumericSchema {
  ...
  bound({ min, max }) {
    return new TlvNumberSchema(this.type, this.lengthProvider, (0, import_Number.maxValue)(min, this.min), (0, import_Number.minValue)(max, this.max));
  }
  validate(value) {
...
const TlvFloat = new TlvNumberSchema(import_TlvCodec.TlvType.Float, (_value) => import_TlvCodec.TlvLength.FourBytes, import_Number.FLOAT32_MIN, import_Number.FLOAT32_MAX);
...
const TlvInt8 = new TlvNumberSchema(
  ...
  import_Number.INT8_MIN,
  import_Number.INT8_MAX
);
const TlvInt16 = new TlvNumberSchema(
  import_TlvCodec.TlvType.SignedInt,
  (value) => import_TlvCodec.TlvCodec.getIntTlvLength(value),
  import_Number.INT16_MIN,
  import_Number.INT16_MAX
);
const TlvInt32 = new TlvNumberSchema(
  import_TlvCodec.TlvType.SignedInt,
  (value) => import_TlvCodec.TlvCodec.getIntTlvLength(value),
  import_Number.INT32_MIN,
  import_Number.INT32_MAX
);
const TlvInt64 = new TlvLongNumberSchema(
  import_TlvCodec.TlvType.SignedInt,
  (value) => import_TlvCodec.TlvCodec.getIntTlvLength(value),
  import_Number.INT64_MIN,
  import_Number.INT64_MAX
);
