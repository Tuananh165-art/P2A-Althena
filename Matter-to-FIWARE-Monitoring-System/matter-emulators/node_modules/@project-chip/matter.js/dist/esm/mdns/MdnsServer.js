...
class MdnsServer {
  ...
  async handleDnsMessage(messageBytes, remoteIp, netInterface) {
    ...
    if (messageType !== DnsMessageType.Query && messageType !== DnsMessageType.TruncatedQuery)
      return;
    if (queries.length === 0)
      return;
    for (const portRecords of records.values()) {
      ...
      await Time.sleep(20 + Math.floor(Math.random() * 100));
    }
  }
      ...
      this.records.keys().map(async (netInterface) => {
        ...
        for (const [portType, portTypeRecords] of records) {
          ...
          portTypeRecords.forEach((record) => {
            record.ttl = 0;
            if (instanceName === void 0 && record.recordType === DnsRecordType.TXT) {
