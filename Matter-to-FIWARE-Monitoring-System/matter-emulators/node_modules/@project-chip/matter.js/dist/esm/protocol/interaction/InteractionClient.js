...
class SubscriptionClient {
  constructor() {
    this.subscriptionListeners = /* @__PURE__ */ new Map();
    this.subscriptionUpdateTimers = /* @__PURE__ */ new Map();
  }
  getId() {
...
class InteractionClient {
  constructor(exchangeProvider, nodeId) {
    ...
    this.subscribedLocalValues = /* @__PURE__ */ new Map();
    this.subscribedClusterDataVersions = /* @__PURE__ */ new Map();
    this.ownSubscriptionIds = /* @__PURE__ */ new Set();
  ...
  async processReadRequest(messenger, request) {
    ...
    const attributeReports = new Array();
    const eventReports = new Array();
    while (true) {
      if (response.attributeReports !== void 0) {
        attributeReports.push(...response.attributeReports);
      }
      if (response.eventReports !== void 0) {
  ...
  async subscribeAttribute(options) {
    ...
    return this.withMessenger(async (messenger) => {
      ...
      const subscriptionListener = (dataReport) => {
        ...
        this.subscribedLocalValues.set(attributePathToId({ endpointId: endpointId2, clusterId: clusterId2, attributeId: attributeId2 }), value);
        this.subscribedClusterDataVersions.set(clusterPathToId({ endpointId: endpointId2, clusterId: clusterId2 }), version);
  ...
  async subscribeMultipleAttributesAndEvents(options) {
    ...
    return this.withMessenger(async (messenger) => {
      logger.debug(
        `Sending subscribe request: attributes: ${attributeRequests.map((path) => resolveAttributeName(path)).join(", ")} and events: ${eventRequests.map((path) => resolveEventName(path)).join(", ")}`
      );
      const {
        report,
        subscribeResponse: { subscriptionId, maxInterval }
  ...
  registerSubscriptionUpdateTimer(subscriptionId, maxInterval, updateTimeoutHandler) {
    ...
    maxInterval += 5;
    const timer = Time.getTimer(maxInterval * 1e3, () => {
