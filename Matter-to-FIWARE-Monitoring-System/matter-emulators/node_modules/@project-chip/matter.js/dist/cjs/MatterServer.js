 ...
 */
const logger = import_Logger.Logger.get("MatterServer");
...
class MatterServer {
  ...
  constructor(storageManager, options) {
    ...
    this.formerlyUsedPorts = new Array();
  }
  get ipv4Disabled() {
  ...
  getNextMatterPort(desiredPort) {
    ...
    for (const node of this.nodes.values()) {
      ...
      if (portCheckMap.has(nodePort)) {
        throw new import_Network.NetworkError(`Port ${nodePort} is already in use by other node.`);
      }
      portCheckMap.set(nodePort, true);
    }
    if (desiredPort !== void 0) {
      if (portCheckMap.has(desiredPort)) {
        throw new import_Network.NetworkError(`Port ${desiredPort} is already in use by other node.`);
      }
      return desiredPort;
    }
    ...
    if (portToCheck === 65536) {
      ...
      if (portToCheck === 65536) {
        throw new import_Network.NetworkError("No free port available for Matter server.");
      }
    }
    return portToCheck;
  }
  ...
  async start() {
    if (this.mdnsBroadcaster === void 0) {
      this.mdnsBroadcaster = await import_MdnsBroadcaster.MdnsBroadcaster.create({
    ...
    if (this.mdnsScanner === void 0) {
      this.mdnsScanner = await import_MdnsScanner.MdnsScanner.create({
        enableIpv4: !this.ipv4Disabled,
