...
function checkDerVersion(type, node, version) {
  const derVersion = node && node._tag === import_DerCodec.DerType.UnsignedInt && node._bytes && node._bytes.length === 1 && node._bytes[0];
...
function getDerCurve(type, node) {
  ...
  const curve = CurveLookup[oid.toHex()];
  if (curve)
    return curve;
...
((Translators2) => {
  Translators2.sec1 = {
    set: function(input) {
      const decoded = import_DerCodec.DerCodec.decode(input);
    ...
    get: function() {
      throw new import_MatterError.NotImplementedError("SEC1 export not implemented");
    }
  };
  Translators2.pkcs8 = {
    set: function(input) {
      const outer = import_DerCodec.DerCodec.decode(input);
      ...
      if (algorithm?.toHex() !== "2a8648ce3d0201" /* ecPublicKey */) {
    ...
    get: function() {
      throw new import_MatterError.NotImplementedError("PKCS #8 export not implemented");
    }
  };
  Translators2.spki = {
    set: function(input) {
    ...
    get: function() {
      throw new import_MatterError.NotImplementedError("SPKI export not implemented");
    }
  };
  Translators2.publicBits = {
    ...
    get: function() {
      ...
      return new import_ByteArray.ByteArray([4, ...this.xBits, ...this.yBits]);
    }
  };
...
function Key(properties) {
  ...
  Object.entries(Base64Codecs).forEach(([alias, target]) => {
    Object.defineProperty(that, alias, {
      get: () => that[target] !== void 0 && import_Base64Codec.Base64.decode(that[target]),
      set: (value) => that[target] = value === void 0 ? void 0 : import_Base64Codec.Base64.encode(value, true)
    });
    assign(alias);
  });
  ...
  function derivePublicFromPrivate() {
    if (that.type !== "EC" /* EC */)
      throw new KeyError("EC key type required to compute public point");
    if (!that.private)
      throw new KeyError("EC private key required to compute public point");
    const crv = that.crv;
    switch (crv) {
      case "P-256" /* p256 */:
      case "P-384" /* p384 */:
    ...
    const elliptic = new import_Crypto.ec(crv.toLowerCase().replace(/-/g, ""));
    const ecKey = elliptic.keyFromPrivate(that.privateKey).getPublic();
    that.xBits = new import_ByteArray.ByteArray(ecKey.getX().toArray());
    that.yBits = new import_ByteArray.ByteArray(ecKey.getY().toArray());
  }
  if (that.type === "EC" /* EC */) {
    if (that.d) {
