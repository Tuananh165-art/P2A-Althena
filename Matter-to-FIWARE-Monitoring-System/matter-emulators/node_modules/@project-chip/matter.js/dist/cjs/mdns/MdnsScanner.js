...
var import_UdpMulticastServer = require("../net/UdpMulticastServer.js");
 ...
 */
const logger = import_Logger.Logger.get("MdnsScanner");
const START_ANNOUNCE_INTERVAL_SECONDS = 1.5;
class MdnsScanner {
  constructor(multicastServer, enableIpv4) {
    this.multicastServer = multicastServer;
    this.enableIpv4 = enableIpv4;
    this.activeAnnounceQueries = /* @__PURE__ */ new Map();
    this.nextAnnounceIntervalSeconds = START_ANNOUNCE_INTERVAL_SECONDS;
    this.operationalDeviceRecords = /* @__PURE__ */ new Map();
    this.commissionableDeviceRecords = /* @__PURE__ */ new Map();
    ...
    this.periodicTimer = import_Time.Time.getPeriodicTimer(60 * 1e3, () => this.expire()).start();
  }
  ...
  async sendQueries() {
    ...
    this.queryTimer = import_Time.Time.getTimer(this.nextAnnounceIntervalSeconds * 1e3, () => this.sendQueries()).start();
    logger.debug(
    ...
    const dnsMessageDataToSend = {
      messageType: import_DnsCodec.DnsMessageType.TruncatedQuery,
      transactionId: 0,
      queries,
      authorities: [],
      answers: [],
    ...
    const emptyDnsMessage = import_DnsCodec.DnsCodec.encode(dnsMessageDataToSend);
    ...
    while (true) {
      if (answersToSend.length > 0) {
        ...
        const nextAnswerEncoded = import_DnsCodec.DnsCodec.encodeRecord(nextAnswer);
        dnsMessageSize += nextAnswerEncoded.length;
        if (dnsMessageSize > import_DnsCodec.MAX_MDNS_MESSAGE_SIZE) {
          ...
          await this.multicastServer.send(import_DnsCodec.DnsCodec.encode(dnsMessageDataToSend));
    ...
    await this.multicastServer.send(
      import_DnsCodec.DnsCodec.encode({ ...dnsMessageDataToSend, messageType: import_DnsCodec.DnsMessageType.Query })
    );
  }
  /**
  ...
  setQueryRecords(queryId, queries, answers = []) {
    ...
    this.queryTimer?.stop();
    this.nextAnnounceIntervalSeconds = START_ANNOUNCE_INTERVAL_SECONDS;
    this.queryTimer = import_Time.Time.getTimer(0, () => this.sendQueries()).start();
  }
  ...
  sortServerEntries(entries) {
    return entries.sort((a, b) => {
      const aIsIPv6 = (0, import_Ip.isIPv6)(a.ip);
      const bIsIPv6 = (0, import_Ip.isIPv6)(b.ip);
      if (aIsIPv6 && !bIsIPv6) {
        return -1;
  ...
  async registerWaiterPromise(queryId, timeoutSeconds, resolveOnUpdatedRecords = true) {
    const { promise, resolver } = (0, import_Promises.createPromise)();
    const timer = timeoutSeconds !== void 0 ? import_Time.Time.getTimer(timeoutSeconds * 1e3, () => this.finishWaiter(queryId, true)).start() : void 0;
  ...
  createOperationalMatterQName(operationalId, nodeId) {
    ...
    return (0, import_MdnsConsts.getDeviceMatterQname)(operationalIdString, import_NodeId.NodeId.toHexString(nodeId));
  }
  /**
  ...
  async findOperationalDevice({ operationalId }, nodeId, timeoutSeconds, ignoreExistingRecords = false) {
    if (this.closing) {
      throw new import_MatterError.ImplementationError("Cannot discover operational device because scanner is closing.");
    }
      ...
      this.setQueryRecords(deviceMatterQname, [
        {
          ...
          recordClass: import_DnsCodec.DnsRecordClass.IN,
          recordType: import_DnsCodec.DnsRecordType.SRV
        }
      ]);
      await promise;
  ...
  cancelOperationalDeviceDiscovery(fabric, nodeId) {
  ...
  buildCommissionableQueryIdentifier(identifier) {
    ...
    throw new import_MatterError.ImplementationError(`Invalid commissionable device identifier : ${JSON.stringify(identifier)}`);
  }
  ...
  findCommissionableQueryIdentifier(instanceName, record) {
    if (this.closing) {
      throw new import_MatterError.ImplementationError("Cannot discover commissionable device because scanner is closing.");
    }
    ...
    if (record.V !== void 0) {
      const vendorIdQueryId = this.buildCommissionableQueryIdentifier({ vendorId: (0, import_VendorId.VendorId)(record.V) });
  ...
  getCommissionableQueryRecords(identifier) {
    ...
    return names.map((name) => ({ name, recordClass: import_DnsCodec.DnsRecordClass.IN, recordType: import_DnsCodec.DnsRecordType.PTR }));
  }
  /**
   ...
   */
  async findCommissionableDevicesContinuously(identifier, callback, timeoutSeconds = 900) {
    const discoveredDevices = /* @__PURE__ */ new Set();
    const discoveryEndTime = import_Time.Time.nowMs() + timeoutSeconds * 1e3;
    const queryId = this.buildCommissionableQueryIdentifier(identifier);
    this.setQueryRecords(queryId, this.getCommissionableQueryRecords(identifier));
    while (true) {
      ...
      const remainingTime = Math.ceil((discoveryEndTime - import_Time.Time.nowMs()) / 1e3);
      if (remainingTime <= 0) {
        break;
      }
  ...
  handleDnsMessage(messageBytes, _remoteIp, netInterface) {
    if (this.closing)
      return;
    const message = import_DnsCodec.DnsCodec.decode(messageBytes);
    if (message === void 0)
      return;
    if (message.messageType !== import_DnsCodec.DnsMessageType.Response && message.messageType !== import_DnsCodec.DnsMessageType.TruncatedResponse)
      return;
    const answers = [...message.answers, ...message.additionalRecords];
    if (this.handleOperationalRecords(answers, this.getActiveQueryEarlierAnswers(), netInterface))
      return;
  ...
  handleIpRecords(answers, target, netInterface) {
    const ipRecords = answers.filter(
      ({ name, recordType }) => (recordType === import_DnsCodec.DnsRecordType.A && this.enableIpv4 || recordType === import_DnsCodec.DnsRecordType.AAAA) && name === target
    );
  ...
  handleOperationalRecords(answers, formerAnswers, netInterface) {
    ...
    const operationalTxtRecord = answers.find(
      ({ name, recordType }) => recordType === import_DnsCodec.DnsRecordType.TXT && name.endsWith(import_MdnsConsts.MATTER_SERVICE_QNAME)
    );
    ...
    const operationalSrvRecord = answers.find(
      ({ name, recordType }) => recordType === import_DnsCodec.DnsRecordType.SRV && name.endsWith(import_MdnsConsts.MATTER_SERVICE_QNAME)
    ) ?? formerAnswers.find(
      ({ name, recordType }) => recordType === import_DnsCodec.DnsRecordType.SRV && name.endsWith(import_MdnsConsts.MATTER_SERVICE_QNAME)
    );
  ...
  handleOperationalTxtRecord(record, netInterface) {
    ...
    if (device !== void 0) {
      device = {
        ...device,
        expires: import_Time.Time.nowMs() + ttl * 1e3,
        ...txtData
      };
    } else {
      logger.debug(
  ...
  handleOperationalSrvRecord(record, answers, formerAnswers, netInterface) {
    ...
    if (ips.length > 0) {
      for (const { value: ip, ttl: ttl2 } of ips) {
        ...
        matterServer.expires = import_Time.Time.nowMs() + ttl2 * 1e3;
    ...
    if (addresses.size === 0 && this.hasWaiter(matterName)) {
      const queries = [{ name: target, recordClass: import_DnsCodec.DnsRecordClass.IN, recordType: import_DnsCodec.DnsRecordType.AAAA }];
      if (this.enableIpv4) {
        queries.push({ name: target, recordClass: import_DnsCodec.DnsRecordClass.IN, recordType: import_DnsCodec.DnsRecordType.A });
      }
  ...
  handleCommissionableRecords(answers, formerAnswers, netInterface) {
    ...
    const txtRecords = commissionableRecords.filter(({ recordType }) => recordType === import_DnsCodec.DnsRecordType.TXT);
    ...
    const srvRecords = commissionableRecords.filter(({ recordType }) => recordType === import_DnsCodec.DnsRecordType.SRV);
    ...
    if (queryMissingDataForInstances.size !== 0) {
      for (const name of Array.from(queryMissingDataForInstances.values())) {
        ...
        this.setQueryRecords(
          queryId,
          [{ name, recordClass: import_DnsCodec.DnsRecordClass.IN, recordType: import_DnsCodec.DnsRecordType.ANY }],
          answers
        );
      }
    }
  ...
  parseCommissionableTxtRecord(record) {
    ...
    if (result.D === void 0 || result.CM === void 0)
      return void 0;
    return result;
  }
  expire() {
    ...
    [...this.operationalDeviceRecords.entries()].forEach(([recordKey, { addresses, expires }]) => {
      if (now < expires) {
        [...addresses.entries()].forEach(([key, { expires: expires2 }]) => {
          if (now < expires2)
            return;
  ...
  static discoveryDataDiagnostics(data) {
    return new import_Logger.DiagnosticDictionary({
      SII: data.SII,
      SAI: data.SAI,
      SAT: data.SAT,
      T: data.T,
  ...
  static deviceAddressDiagnostics(addresses) {
    return Array.from(addresses.values()).map(
      (address) => new import_Logger.DiagnosticDictionary({
        ip: address.ip,
        port: address.port,
        type: address.type
      })
