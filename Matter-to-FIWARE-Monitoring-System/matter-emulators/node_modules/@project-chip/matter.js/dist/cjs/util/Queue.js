...
class Queue {
  ...
  async read(timeoutMs = 6e4) {
    ...
    if (this.closed)
      throw new import_Stream.EndOfStreamError();
    const data = this.queue.shift();
    if (data !== void 0) {
      return data;
    }
    if (this.pendingRead !== void 0)
      throw new import_MatterError.MatterFlowError("Only one pending read is supported");
    this.pendingRead = {
      resolver,
      rejecter,
      timeoutTimer: import_Time.Time.getTimer(timeoutMs, () => rejecter(new import_Stream.NoResponseTimeoutError())).start()
    };
    return promise;
  }
  async write(data) {
