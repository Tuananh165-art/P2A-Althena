...
var ModelVariantTraversal_exports = {};
__export(ModelVariantTraversal_exports, {
  ModelVariantTraversal: () => ModelVariantTraversal
});
module.exports = __toCommonJS(ModelVariantTraversal_exports);
 ...
 */
const logger = import_Logger.Logger.get("ModelVariantTraversal");
class ModelVariantTraversal {
   ...
   */
  constructor(sourceNames) {
    this.sourceNames = sourceNames;
    this.visiting = false;
    this.modelTraversal = new import_ModelTraversal.ModelTraversal();
  }
  /**
  ...
  traverse(variants) {
    if (this.visiting) {
      throw new import_MatterError.InternalError("ModelVariantVisitor.visit called with active visit; reentrancy not supported");
    }
        ...
        this.createVariantDetail(
          Object.fromEntries(
            Object.entries(variants).map(([sourceName, element]) => {
              if (!(element instanceof import_models.Model)) {
                element = import_models.Model.create(element);
              }
  ...
  enterCluster(variants) {
    if (variants.tag === import_definitions.ElementTag.Cluster) {
      this.clusterState = {
    ...
    const state = this.modelTraversal.operation(() => {
      return this.visit(variants, () => {
        ...
        for (const mapping of Object.values(mappings)) {
          mappings:
            for (const childVariants of mapping.slots) {
              ...
              if (variants.tag === import_definitions.ElementTag.Cluster) {
                for (const k in variants.map) {
                  ...
                  if (child instanceof import_models.ValueModel && child.overridesShadow) {
                    overrides = true;
                  }
                }
              }
  ...
  mapChildren(variants) {
    ...
    for (const [sourceName, variant] of Object.entries(variants.map)) {
      for (let i = 0; i < variant.children.length; i++) {
        ...
        if (childId !== void 0) {
          if (mapping.idToSlot[childId] === void 0) {
            mapping.idToSlot[childId] = slot;
          }
        }
        if (mapping.nameToSlot[childName] === void 0) {
  ...
  createVariantDetail(map) {
    ...
    if (!tag) {
      throw new import_MatterError.InternalError("No tag identified in variant set");
    }
    if (!name) {
      throw new import_MatterError.InternalError("No name identified in variant set");
    }
    return { tag, id, name, map };
  }
}
function computeCanonicalNames(sourceNames, variants) {
...
function inferEquivalentDatatypes(sourceNames, variants, datatypeNameMap) {
  ...
  const traversal = new class extends ModelVariantTraversal {
    visit(variants2, recurse) {
      ...
      for (let priority = 0; priority < sourceNames.length; priority++) {
        ...
        if (!base || base.parent?.tag !== import_definitions.ElementTag.Cluster) {
          continue;
        }
        if (!mapEntry) {
          mapEntry = {
...
function chooseCanonicalNames(sourceNames, variants, datatypeNameMap) {
  ...
  const traversal = new class extends ModelVariantTraversal {
    ...
    enterCluster(variants2) {
      if (variants2.tag === import_definitions.ElementTag.Cluster) {
        this.clusterState = { canonicalNames: datatypeNameMap };
        return true;
      }
      return false;
