...
class PairedNode {
  constructor(nodeId, commissioningController, options = {}, reconnectInteractionClient, assignDisconnectedHandler) {
    ...
    this.reconnectDelayTimer = Time.getTimer(
      STRUCTURE_UPDATE_TIMEOUT_MS,
      async () => await this.reconnect()
    );
    this.updateEndpointStructureTimer = Time.getTimer(
      5e3,
  ...
  async openBasicCommissioningWindow(commissioningTimeout = 900) {
    ...
    try {
      ...
      if (!(error instanceof StatusResponseError) || error.code !== StatusCode.Failure || error.clusterCode !== AdministratorCommissioning.StatusCode.WindowNotOpen) {
        throw error;
      }
    }
