 ...
 */
class BtpMatterError extends import_MatterError.MatterError {
}
...
const logger = import_Logger.Logger.get("BtpSessionHandler");
class BtpSessionHandler {
  ...
  constructor(role, btpVersion, fragmentSize, clientWindowSize, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    this.prevIncomingSequenceNumber = 255;
    // Incoming Sequence Number received. Set to 255 to start at 0
    this.prevIncomingAckNumber = -1;
    // Previous ackNumber received
    this.ackReceiveTimer = import_Time.Time.getTimer(import_BleConsts.BTP_ACK_TIMEOUT_MS, () => this.btpAckTimeoutTriggered());
    this.sequenceNumber = 0;
    // Sequence number is set to 0 already for the handshake, next sequence number is 1
    this.prevAckedSequenceNumber = -1;
    // Previous (outgoing) Acked Sequence Number
    this.queuedOutgoingMatterMessages = new Array();
    this.sendInProgress = false;
    this.sendAckTimer = import_Time.Time.getTimer(import_BleConsts.BTP_SEND_ACK_TIMEOUT_MS, () => this.btpSendAckTimeoutTriggered());
    this.isActive = true;
    if (btpVersion !== 4) {
  ...
  static async createFromHandshakeRequest(maxDataSize, handshakeRequestPayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    logger.debug(
      `Sending BTP packet: ${import_Logger.Logger.dict({
        version,
        attMtu,
        windowSize
      })}`
  ...
  static async createAsCentral(handshakeResponsePayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    const fragmentSize = handshakeMtu - 3;
  ...
  async handleIncomingBleData(data) {
    ...
    try {
      ...
      logger.debug(`Received BTP packet: ${import_Logger.Logger.toJSON(btpPacket)}`);
      const {
        header: {
          hasAckNumber,
          isHandshakeRequest,
      ...
      if (isBeginningSegment) {
        ...
        this.currentIncomingSegmentedPayload = import_ByteArray.ByteArray.concat(
  ...
  async sendMatterMessage(data) {
    ...
    const dataReader = new import_DataReader.DataReader(data, import_ByteArray.Endian.Little);
  ...
  async processSendQueue() {
    ...
    if (this.exceedsWindowSize(this.prevIncomingAckNumber, this.sequenceNumber))
      return;
    if (this.queuedOutgoingMatterMessages.length === 0)
      return;
    ...
    while (this.queuedOutgoingMatterMessages.length > 0) {
      ...
      logger.debug(
        `Sending BTP fragment: ${import_Logger.Logger.dict({
      ...
      logger.debug(`Sending BTP packet: ${import_Logger.Logger.toJSON(btpPacket)}`);
