...
class CaseServer {
  ...
  async handleSigma1(server, messenger) {
    ...
    if (peerResumptionId !== void 0 && peerResumeMic !== void 0 && resumptionRecord !== void 0) {
      ...
      logger.info(
        `session ${secureSession.getId()} resumed with ${messenger.getChannelName()} for Fabric ${import_NodeId.NodeId.toHexString(
          fabric.nodeId
      ...
      const { publicKey: ecdhPublicKey, sharedSecret } = import_Crypto.Crypto.ecdhGeneratePublicKeyAndSecret(peerEcdhPublicKey);
      ...
      const peerEncryptedData = import_Crypto.Crypto.decrypt(sigma3Key, peerEncrypted, import_CaseMessages.TBE_DATA3_NONCE);
      const {
        nodeOpCert: peerNewOpCert,
        intermediateCACert: peerIntermediateCACert,
        signature: peerSignature
      } = import_CaseMessages.TlvEncryptedDataSigma3.decode(peerEncryptedData);
      ...
      logger.info(
        `session ${secureSession.getId()} created with ${messenger.getChannelName()} for Fabric ${import_NodeId.NodeId.toHexString(
          fabric.nodeId
        )}(index ${fabric.fabricIndex}) and PeerNode ${import_NodeId.NodeId.toHexString(peerNodeId)}`
      );
      ...
      await messenger.close();
      server.saveResumptionRecord(resumptionRecord2);
    } else {
      logger.info(
