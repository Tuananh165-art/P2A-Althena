...
class MessageCodec {
  ...
  static decodePayload({ header, applicationPayload }) {
    const reader = new import_DataReader.DataReader(applicationPayload, import_ByteArray.Endian.Little);
  ...
  static encodePayload({ packetHeader, payloadHeader, payload, securityExtension }) {
    if (securityExtension !== void 0 || payloadHeader.hasSecuredExtension) {
      throw new import_MatterError.NotImplementedError(`Security extensions not supported when encoding a payload.`);
    }
    return {
      header: packetHeader,
      applicationPayload: import_ByteArray.ByteArray.concat(this.encodePayloadHeader(payloadHeader), payload)
    };
  }
  static encodePacket({ header, applicationPayload, messageExtension }) {
    if (messageExtension !== void 0 || header.hasMessageExtensions) {
      throw new import_MatterError.NotImplementedError(`Message extensions not supported when encoding a packet.`);
    }
    return import_ByteArray.ByteArray.concat(this.encodePacketHeader(header), applicationPayload);
  }
  static decodePacketHeader(reader) {
    ...
    if (version !== HEADER_VERSION)
      throw new import_MatterError.NotImplementedError(`Unsupported header version ${version}.`);
    const sessionId = reader.readUInt16();
    const securityFlags = reader.readUInt8();
    const messageId = reader.readUInt32();
    const sourceNodeId = hasSourceNodeId ? (0, import_NodeId.NodeId)(reader.readUInt64()) : void 0;
    const destNodeId = hasDestNodeId ? (0, import_NodeId.NodeId)(reader.readUInt64()) : void 0;
    const destGroupId = hasDestGroupId ? (0, import_GroupId.GroupId)(reader.readUInt16()) : void 0;
    const sessionType = securityFlags & 3;
    if (sessionType !== 1 /* Group */ && sessionType !== 0 /* Unicast */)
      throw new import_MatterError.UnexpectedDataError(`Unsupported session type ${sessionType}`);
    const hasPrivacyEnhancements = (securityFlags & 128 /* HasPrivacyEnhancements */) !== 0;
    if (hasPrivacyEnhancements)
      throw new import_MatterError.NotImplementedError(`Privacy enhancements not supported`);
    const isControlMessage = (securityFlags & 64 /* IsControlMessage */) !== 0;
    if (isControlMessage)
      throw new import_MatterError.NotImplementedError(`Control Messages not supported`);
  ...
  static encodePacketHeader({
    ...
    if (sourceNodeId !== void 0)
      writer.writeUInt64(sourceNodeId);
    if (destNodeId !== void 0)
      writer.writeUInt64(destNodeId);
    if (destGroupId !== void 0)
