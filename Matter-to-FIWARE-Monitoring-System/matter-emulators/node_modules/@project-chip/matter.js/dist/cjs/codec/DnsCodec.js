...
class DnsCodec {
  static decode(message) {
    try {
      const reader = new import_DataReader.DataReader(message, import_ByteArray.Endian.Big);
  ...
  static decodeQName(reader, message) {
    const messageReader = new import_DataReader.DataReader(message, import_ByteArray.Endian.Big);
  ...
  static decodeRecordValue(valueBytes, recordType, message) {
    switch (recordType) {
      case 12 /* PTR */:
        return this.decodeQName(new import_DataReader.DataReader(valueBytes, import_ByteArray.Endian.Big), message);
      case 33 /* SRV */:
  ...
  static decodeSrvRecord(valueBytes, message) {
    const reader = new import_DataReader.DataReader(valueBytes, import_ByteArray.Endian.Big);
  ...
  static encode({
    ...
    if (messageType === void 0)
      throw new import_MatterError.InternalError("Message type must be specified!");
    ...
    if (authorities.length > 0)
      throw new import_MatterError.NotImplementedError("Authority answers are not supported yet!");
    const writer = new import_DataWriter.DataWriter(import_ByteArray.Endian.Big);
    ...
    [...answers, ...authorities, ...additionalRecords].forEach((record) => {
      if (record instanceof import_ByteArray.ByteArray) {
  ...
  static encodeARecord(ip) {
    if (!(0, import_Ip.isIPv4)(ip))
      throw new import_MatterError.UnexpectedDataError(`Invalid A Record value: ${ip}`);
    const writer = new import_DataWriter.DataWriter(import_ByteArray.Endian.Big);
  ...
  static encodeAaaaRecord(ip) {
    if (!(0, import_Ip.isIPv6)(ip))
      throw new import_MatterError.UnexpectedDataError(`Invalid AAAA Record value: ${ip}`);
    const writer = new import_DataWriter.DataWriter(import_ByteArray.Endian.Big);
  ...
  static encodeTxtRecord(entries) {
    ...
    entries.forEach((entry) => {
      const entryData = import_ByteArray.ByteArray.fromString(entry);
  ...
  static encodeQName(qname) {
    ...
    if (qname.length > 0) {
      qname.split(".").forEach((label) => {
        const labelData = import_ByteArray.ByteArray.fromString(label);
