 ...
 */
const logger = import_Logger.Logger.get("EventDataDecoder");
...
function normalizeAndDecodeEventData(data) {
  ...
  responseList.forEach((values) => {
    ...
    if (endpointId === void 0 || clusterId === void 0 || eventId === void 0) {
      throw new import_MatterError.UnexpectedDataError(`Invalid event path ${endpointId}/${clusterId}/${eventId}`);
    }
    try {
      const cluster = (0, import_ClusterHelper.getClusterById)(clusterId);
      const eventDetail = (0, import_ClusterHelper.getClusterEventById)(cluster, eventId);
      if (eventDetail === void 0) {
        logger.debug(
          `Decode unknown event ${(0, import_Number.toHexString)(clusterId)}/${(0, import_Number.toHexString)(eventId)} via the AnySchema.`
        );
        const eventName = `Unknown (${(0, import_Number.toHexString)(eventId)})`;
...
function decodeUnknownEventValue(data) {
  const schema = import_TlvAny.TlvAny;
  if (data.length === 0) {
    return void 0;
  }
