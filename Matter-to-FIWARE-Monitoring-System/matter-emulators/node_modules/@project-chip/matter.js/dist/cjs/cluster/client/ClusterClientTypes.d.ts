...
import { Attribute, AttributeJsType, Attributes, Cluster, Command, Commands, EventType, Events, GlobalAttributeNames, GlobalAttributes, MandatoryAttributeNames, MandatoryEventNames, OptionalAttribute, OptionalAttributeNames, OptionalEventNames, OptionalWritableAttribute, RequestType, ResponseType, WritableAttribute } from "../Cluster.js";
...
export type AttributeClients<F extends BitSchema, A extends Attributes> = Merge<Merge<{
...
export type SignatureFromCommandSpec<C extends Command<any, any, any>> = (request: RequestType<C>, options?: {
...
type ClientAttributeGetters<A extends Attributes> = Omit<{
    [P in keyof A as `get${Capitalize<string & P>}Attribute`]: (alwaysRequestFromRemote?: boolean, isFabricFiltered?: boolean) => Promise<GetterTypeFromSpec<A[P]>>;
}, keyof GlobalAttributes<any>>;
type ClientGlobalAttributeGetters<F extends BitSchema> = {
    [P in GlobalAttributeNames<F> as `get${Capitalize<string & P>}Attribute`]: () => Promise<GetterTypeFromSpec<GlobalAttributes<F>[P]>>;
};
type WritableAttributeNames<A extends Attributes> = {
    [K in keyof A]: A[K] extends WritableAttribute<any, any> ? K : never;
}[keyof A] | {
    [K in keyof A]: A[K] extends OptionalWritableAttribute<any, any> ? K : never;
}[keyof A];
type ClientAttributeSetters<A extends Attributes> = {
    [P in WritableAttributeNames<A> as `set${Capitalize<string & P>}Attribute`]: (value: AttributeJsType<A[P]>) => Promise<void>;
};
type ClientAttributeSubscribers<A extends Attributes> = {
    [P in keyof A as `subscribe${Capitalize<string & P>}Attribute`]: (listener: (value: AttributeJsType<A[P]>) => void, minIntervalS: number, maxIntervalS: number, knownDataVersion?: number, isFabricFiltered?: boolean) => Promise<void>;
};
type ClientAttributeListeners<A extends Attributes> = {
    [P in keyof A as `add${Capitalize<string & P>}AttributeListener`]: (listener: (value: AttributeJsType<A[P]>) => void) => void;
};
type CommandServers<C extends Commands> = {
    [P in keyof C]: SignatureFromCommandSpec<C[P]>;
};
type ClientEventGetters<E extends Events> = {
    [P in keyof E as `get${Capitalize<string & P>}Event`]: (minimumEventNumber?: number | bigint, isFabricFiltered?: boolean) => Promise<DecodedEventData<EventType<E[P]>>>;
};
type ClientEventSubscribers<E extends Events> = {
    [P in keyof E as `subscribe${Capitalize<string & P>}Event`]: (listener: (value: DecodedEventData<EventType<E[P]>>) => void, minIntervalS: number, maxIntervalS: number, isUrgent?: boolean, minimumEventNumber?: number | bigint, isFabricFiltered?: boolean) => Promise<void>;
};
type ClientEventListeners<E extends Events> = {
    [P in keyof E as `add${Capitalize<string & P>}EventListener`]: (listener: (value: DecodedEventData<EventType<E[P]>>) => void) => void;
};
...
} & ClientAttributeGetters<A> & ClientGlobalAttributeGetters<F> & ClientAttributeSetters<A> & ClientAttributeSubscribers<A> & ClientAttributeListeners<A> & CommandServers<C> & ClientEventGetters<E> & ClientEventSubscribers<E> & ClientEventListeners<E>;
export type ClusterClientObjInternal<F extends BitSchema, A extends Attributes, C extends Commands, E extends Events> = ClusterClientObj<F, A, C, E> & {
    /**
...
export declare function isClusterClient<F extends BitSchema, A extends Attributes, C extends Commands, E extends Events>(obj: ClusterClientObj<F, A, C, E> | ClusterServerObj<A, E>): obj is ClusterClientObj<F, A, C, E>;
export declare function isClusterClientInternal<F extends BitSchema, A extends Attributes, C extends Commands, E extends Events>(obj: ClusterClientObj<F, A, C, E> | ClusterServerObj<A, E>): obj is ClusterClientObjInternal<F, A, C, E>;
export declare function asClusterClientInternal<F extends BitSchema, A extends Attributes, C extends Commands, E extends Events>(obj: ClusterClientObj<F, A, C, E>): ClusterClientObjInternal<F, A, C, E>;
export {};
//# sourceMappingURL=ClusterClientTypes.d.ts.map