...
var import_BindingCluster = require("../cluster/definitions/BindingCluster.js");
...
class PairedDevice extends import_Endpoint.Endpoint {
  ...
  addClusterServer(cluster) {
    if (this.declineAddingMoreClusters) {
      throw new import_MatterError.ImplementationError("PairedDevice does not support adding additional clusters");
    }
...
class RootEndpoint extends import_Endpoint.Endpoint {
   ...
   */
  constructor() {
    super([import_DeviceTypes.DeviceTypes.ROOT], { endpointId: (0, import_EndpointNumber.EndpointNumber)(0) });
    this.deviceType = import_DeviceTypes.DeviceTypes.ROOT.code;
  }
  /**
...
class Device extends import_Endpoint.Endpoint {
  ...
  constructor(definition, options = {}) {
    if (definition.deviceClass === import_DeviceTypes.DeviceClasses.Node) {
      throw new import_MatterError.NotImplementedError("MatterNode devices are not supported");
    }
    ...
    if (definition.deviceClass === import_DeviceTypes.DeviceClasses.Simple || definition.deviceClass === import_DeviceTypes.DeviceClasses.Client) {
      this.addClusterServer(
        (0, import_ClusterServer.ClusterServer)(
          import_BindingCluster.Binding.Cluster,
          {
            binding: []
          },
          {}
  ...
  createOptionalClusterServer(_cluster) {
    throw new import_MatterError.ImplementationError("createOptionalClusterServer needs to be implemented by derived classes");
  }
  createOptionalClusterClient(_cluster) {
    throw new import_MatterError.ImplementationError("createOptionalClusterClient needs to be implemented by derived classes");
  }
  ...
  setBridgedDeviceReachability(reachable) {
    const bridgedBasicInformationCluster = this.getClusterServer(import_BridgedDeviceBasicInformationCluster.BridgedDeviceBasicInformationCluster);
