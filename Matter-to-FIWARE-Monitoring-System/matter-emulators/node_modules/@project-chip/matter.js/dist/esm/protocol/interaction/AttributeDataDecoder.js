 ...
 */
import { AttributeError } from "../../cluster/Cluster.js";
...
function normalizeAndDecodeReadAttributeReport(data) {
  ...
  return normalizeAndDecodeAttributeData(dataValues);
}
function normalizeAttributeData(data, acceptWildcardPaths = false) {
  ...
  data.forEach((value) => {
    ...
    if (path.enableTagCompression) {
      ...
      if (path.nodeId === void 0 && lastPath.nodeId !== void 0)
        path.nodeId = lastPath.nodeId;
      if (path.endpointId === void 0)
        path.endpointId = lastPath.endpointId;
      if (path.clusterId === void 0)
        path.clusterId = lastPath.clusterId;
      if (path.attributeId === void 0)
        path.attributeId = lastPath.attributeId;
...
function normalizeAndDecodeAttributeData(data) {
  ...
  responseList.forEach((values) => {
    ...
    try {
      ...
      logger.error(
        `Error decoding attribute ${endpointId}/${toHexString(clusterId)}/${toHexString(attributeId)}: ${error.message}`
      );
    }
  });
  return result;
