...
class AnySchema extends import_TlvSchema.TlvSchema {
    ...
    switch (typeLength.type) {
      ...
      case import_TlvCodec.TlvType.List:
        ...
        while (true) {
          ...
          if (typeLengthElement.type === import_TlvCodec.TlvType.EndOfContainer)
            break;
        }
        break;
      case import_TlvCodec.TlvType.EndOfContainer:
  ...
  validate(tlvStream) {
    if (!Array.isArray(tlvStream))
      throw new import_ValidationError.ValidationError(`Expected TlvStream, got ${typeof tlvStream}.`);
    tlvStream.forEach(({ typeLength }) => {
      if (!typeLength || typeof typeLength !== "object")
        throw new import_ValidationError.ValidationError(`Expected typeLength properties in TlvStream, got ${typeof typeLength}.`);
  ...
  decodeAnyTlvStream(encoded) {
    ...
    if (nextElement !== void 0) {
      throw new import_MatterError.UnexpectedDataError(`Unexpected data left after parsing all data: ${import_Logger.Logger.toJSON(nextElement)}`);
    }
    return result;
  }
  decodeGenericElement(reader, preReadElement, allowTag = false) {
    while (true) {
      ...
      switch (type) {
        ...
        case import_TlvCodec.TlvType.ByteString: {
          if (tag !== void 0 && !allowTag) {
            throw new import_MatterError.UnexpectedDataError(
              `Tag detected or invalid length for a native type: ${import_Logger.Logger.toJSON(element)}`
            );
          }
        ...
        default:
          throw new import_MatterError.UnexpectedDataError(`Unknown type: ${type}`);
      }
    }
  }
  decodeGenericArrayOrList(reader) {
    ...
    while (true) {
      ...
      if (type === import_TlvCodec.TlvType.EndOfContainer)
        break;
      if (tag !== void 0) {
        throw new import_MatterError.UnexpectedDataError(
          `Tag detected or invalid length for a native type: ${import_Logger.Logger.toJSON(element)}`
  ...
  decodeGenericStructure(reader) {
    ...
    while (true) {
      ...
      if (tag === void 0 || tag.id === void 0) {
        throw new import_MatterError.UnexpectedDataError(
          `Tag not detected or invalid length for a structure: ${import_Logger.Logger.toJSON(element)}`
        );
      }
