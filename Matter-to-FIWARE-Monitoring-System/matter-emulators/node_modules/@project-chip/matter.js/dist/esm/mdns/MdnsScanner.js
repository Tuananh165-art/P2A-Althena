...
class MdnsScanner {
  constructor(multicastServer, enableIpv4) {
    ...
    this.periodicTimer = Time.getPeriodicTimer(60 * 1e3, () => this.expire()).start();
  }
  ...
  async sendQueries() {
    ...
    this.queryTimer = Time.getTimer(this.nextAnnounceIntervalSeconds * 1e3, () => this.sendQueries()).start();
    logger.debug(
  ...
  async registerWaiterPromise(queryId, timeoutSeconds, resolveOnUpdatedRecords = true) {
    ...
    const timer = timeoutSeconds !== void 0 ? Time.getTimer(timeoutSeconds * 1e3, () => this.finishWaiter(queryId, true)).start() : void 0;
  ...
  buildCommissionableQueryIdentifier(identifier) {
    ...
    throw new ImplementationError(`Invalid commissionable device identifier : ${JSON.stringify(identifier)}`);
  }
  ...
  handleOperationalSrvRecord(record, answers, formerAnswers, netInterface) {
    ...
    if (ips.length > 0) {
      for (const { value: ip, ttl: ttl2 } of ips) {
        ...
        matterServer.expires = Time.nowMs() + ttl2 * 1e3;
  ...
  static deviceAddressDiagnostics(addresses) {
    return Array.from(addresses.values()).map(
      (address) => new DiagnosticDictionary({
        ip: address.ip,
        port: address.port,
        type: address.type
      })
