"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var CommandServer_exports = {};
__export(CommandServer_exports, {
  CommandServer: () => CommandServer
});
module.exports = __toCommonJS(CommandServer_exports);
var import_Logger = require("../../log/Logger.js");
var import_model = require("../../model/index.js");
var import_StatusCode = require("../../protocol/interaction/StatusCode.js");
/**
 * @license
 * Copyright 2022-2023 Project CHIP Authors
 * SPDX-License-Identifier: Apache-2.0
 */
const logger = import_Logger.Logger.get("CommandServer");
class CommandServer {
  constructor(invokeId, responseId, name, requestSchema, responseSchema, requiresTimedInteraction, handler) {
    this.invokeId = invokeId;
    this.responseId = responseId;
    this.name = name;
    this.requestSchema = requestSchema;
    this.responseSchema = responseSchema;
    this.requiresTimedInteraction = requiresTimedInteraction;
    this.handler = handler;
  }
  async invoke(session, args, message, endpoint) {
    let request = this.requestSchema.decodeTlv(args);
    if (session.isSecure()) {
      const fabric = session.getFabric();
      if (fabric) {
        request = this.requestSchema.injectField(
          request,
          import_model.Globals.FabricIndex.id,
          session.getAssociatedFabric().fabricIndex,
          (existingFieldIndex) => existingFieldIndex === void 0
        );
      }
    }
    this.requestSchema.validate(request);
    logger.debug(`Invoke ${this.name} with data ${import_Logger.Logger.toJSON(request)}`);
    const response = await this.handler(request, session, message, endpoint);
    logger.debug(`Invoke ${this.name} response : ${import_Logger.Logger.toJSON(response)}`);
    return {
      code: import_StatusCode.StatusCode.Success,
      responseId: this.responseId,
      response: this.responseSchema.encodeTlv(response)
    };
  }
}
//# sourceMappingURL=CommandServer.js.map
