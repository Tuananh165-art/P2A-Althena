...
var import_BridgedDeviceBasicInformationCluster = require("../cluster/definitions/BridgedDeviceBasicInformationCluster.js");
var import_DescriptorCluster = require("../cluster/definitions/DescriptorCluster.js");
var import_FixedLabelCluster = require("../cluster/definitions/FixedLabelCluster.js");
var import_UserLabelCluster = require("../cluster/definitions/UserLabelCluster.js");
...
class Endpoint {
  ...
  constructor(deviceTypes, options = {}) {
    ...
    this.clusterServers = /* @__PURE__ */ new Map();
    this.clusterClients = /* @__PURE__ */ new Map();
    this.childEndpoints = [];
    this.name = "";
    this.structureChangedCallback = () => {
    };
    this.descriptorCluster = (0, import_ClusterServer.ClusterServer)(
      import_DescriptorCluster.DescriptorCluster,
      {
  ...
  destroy() {
    for (const clusterServer of this.clusterServers.values()) {
      (0, import_ClusterServerTypes.asClusterServerInternal)(clusterServer)._destroy();
    }
  }
  getId() {
    if (this.id === void 0) {
      throw new import_MatterError.InternalError("Endpoint has not been assigned yet");
    }
    return this.id;
  }
  addFixedLabel(label, value) {
    if (!this.hasClusterServer(import_FixedLabelCluster.FixedLabelCluster)) {
      this.addClusterServer(
        (0, import_ClusterServer.ClusterServer)(
          import_FixedLabelCluster.FixedLabelCluster,
          {
            labelList: []
          },
          {}
    ...
    const fixedLabelCluster = this.getClusterServer(import_FixedLabelCluster.FixedLabelCluster);
  ...
  addUserLabel(label, value) {
    if (!this.hasClusterServer(import_UserLabelCluster.UserLabelCluster)) {
      this.addClusterServer(
        (0, import_ClusterServer.ClusterServer)(
          import_UserLabelCluster.UserLabelCluster,
          {
            labelList: []
          },
          {}
    ...
    const userLabelCluster = this.getClusterServer(import_UserLabelCluster.UserLabelCluster);
  ...
  addClusterServer(cluster) {
    ...
    if (currentCluster !== void 0) {
      (0, import_ClusterServerTypes.asClusterServerInternal)(currentCluster)._destroy();
    }
    (0, import_ClusterServerTypes.asClusterServerInternal)(cluster)._assignToEndpoint(this);
    if (cluster.id === import_DescriptorCluster.DescriptorCluster.id) {
  ...
  addChildEndpoint(endpoint) {
    if (endpoint.id !== void 0 && this.getChildEndpoint(endpoint.id) !== void 0) {
      throw new import_MatterError.ImplementationError(`Endpoint with id ${endpoint.id} already exists as child from ${this.id}.`);
    }
  ...
  removeChildEndpoint(endpoint) {
    ...
    if (index === -1) {
      throw new import_MatterError.ImplementationError(`Provided endpoint for deletion does not exist as child endpoint.`);
    }
  ...
  determineUniqueID() {
    ...
    const basicInformationCluster = this.getClusterServer(import_BasicInformationCluster.BasicInformationCluster) ?? this.getClusterServer(import_BridgedDeviceBasicInformationCluster.BridgedDeviceBasicInformationCluster);
    ...
    this.deviceTypes.forEach((deviceType) => {
      deviceType.requiredServerClusters?.forEach((clusterId) => {
        if (!this.clusterServers.has(clusterId)) {
          const clusterName = (0, import_ClusterHelper.getClusterNameById)(clusterId);
      ...
      if (this.clusterClients.size > 0) {
        throw new import_MatterError.NotImplementedError(`Devices with client clusters are not supported yet`);
      }
  ...
  updatePartsList() {
    ...
    for (const child of this.childEndpoints) {
      ...
      if (child.id === void 0) {
        throw new import_MatterError.InternalError(`Child endpoint has no id, can not add to parts list`);
      }
      newPartsList.push((0, import_EndpointNumber.EndpointNumber)(child.id));
