...
var ReedSolomon_exports = {};
__export(ReedSolomon_exports, {
  ReedSolomon: () => ReedSolomon
});
module.exports = __toCommonJS(ReedSolomon_exports);
 ...
 */
class GaloisField {
  constructor(prime = 285, generator = 2, c_exp = 8) {
    this.prime = prime;
    this.exp = new Array();
    this.log = new Array();
...
class ReedSolomon {
  ...
  computeErrorCorrection(data, ecLength) {
    ...
    if (length + ecLength > this.galoisField.size)
      throw new import_MatterError.UnexpectedDataError("Message is too long");
    const generator = this.generatePolynom(ecLength);
    const { length: generatorLength } = generator;
    const buffer = new import_ByteArray.ByteArray(length + generatorLength - 1);
    buffer.set(data, 0);
