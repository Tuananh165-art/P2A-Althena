...
class StringSchema extends import_TlvSchema.TlvSchema {
  constructor(type, minLength = 0, maxLength = 1024) {
    ...
    if (minLength < 0)
      throw new import_MatterError.InternalError("Minimum length should be a positive number.");
  }
  ...
  decodeTlvInternalValue(reader, typeLength) {
    if (typeLength.type !== this.type)
      throw new import_MatterError.UnexpectedDataError(`Unexpected type ${typeLength.type}.`);
    return reader.readPrimitive(typeLength);
  }
  validate(value) {
    if (this.type === import_TlvCodec.TlvType.Utf8String && typeof value !== "string")
      throw new import_ValidationError.ValidationError(`Expected string, got ${typeof value}.`);
    if (this.type === import_TlvCodec.TlvType.ByteString && !(value instanceof import_ByteArray.ByteArray))
      throw new import_ValidationError.ValidationError(`Expected ByteArray, got ${typeof value}.`);
    if (value.length > this.maxLength)
      throw new import_ValidationError.ValidationError(`String is too long: ${value.length}, max ${this.maxLength}.`);
    if (value.length < this.minLength)
      throw new import_ValidationError.ValidationError(`String is too short: ${value.length}, min ${this.minLength}.`);
  }
  bound({ minLength, maxLength, length }) {
    return new StringSchema(
      this.type,
      length ?? (0, import_Number.maxValue)(this.minLength, minLength),
      length ?? (0, import_Number.minValue)(this.maxLength, maxLength)
    );
  }
}
