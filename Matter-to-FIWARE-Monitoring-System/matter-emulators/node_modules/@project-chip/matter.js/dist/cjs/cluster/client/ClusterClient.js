 ...
 */
const logger = import_Logger.Logger.get("ClusterClient");
function ClusterClient(clusterDef, endpointId, interactionClient, globalAttributeValues = {}) {
  function addAttributeToResult(attribute, attributeName) {
    ...
    result[`get${capitalizedAttributeName}Attribute`] = async (alwaysRequestFromRemote, isFabricFiltered = true) => {
      return await (0, import_TryCatchHandler.tryCatchAsync)(
        async () => {
          return await attributes[attributeName].get(isFabricFiltered, alwaysRequestFromRemote);
        },
        import_StatusCode.StatusResponseError,
        (e) => {
          const { code } = e;
          if (code === import_StatusCode.StatusCode.UnsupportedAttribute) {
            return void 0;
          }
          throw e;
        }
    ...
    result[`subscribe${capitalizedAttributeName}Attribute`] = async (listener, minIntervalS, maxIntervalS, knownDataVersion, isFabricFiltered) => {
      ...
      attributes[attributeName].subscribe(
        minIntervalS,
        maxIntervalS,
        knownDataVersion,
        isFabricFiltered
  ...
  function addEventToResult(event, eventName) {
    events[eventName] = (0, import_EventClient.createEventClient)(
      event,
      eventName,
      endpointId,
      clusterId,
    ...
    result[`get${capitalizedEventName}Event`] = async (minimumEventNumber, isFabricFiltered) => {
      return await (0, import_TryCatchHandler.tryCatchAsync)(
        ...
        (e) => {
          const { code } = e;
          if (code === import_StatusCode.StatusCode.UnsupportedEvent) {
            return void 0;
          }
          throw e;
        }
  ...
  const attributeToId = {};
  const allAttributeDefs = (0, import_Type.Merge)(attributeDef, (0, import_Cluster.GlobalAttributes)(features));
  ...
  if (globalAttributeValues?.attributeList !== void 0) {
    for (const attributeId of globalAttributeValues.attributeList) {
      if (attributeToId[attributeId] === void 0) {
        const attribute = (0, import_Cluster.UnknownAttribute)(attributeId);
        addAttributeToResult(attribute, `unknownAttribute_${(0, import_Number.toHexString)(attributeId)}`);
        logger.info(`Added unknown attribute ${(0, import_Number.toHexString)(attributeId)} to cluster ${(0, import_Number.toHexString)(clusterId)}`);
      }
    }
  }
  const eventToId = {};
  ...
  if (globalAttributeValues?.eventList !== void 0) {
    for (const eventId of globalAttributeValues.eventList) {
      if (eventToId[eventId] === void 0) {
        const event = (0, import_Cluster.UnknownEvent)(eventId);
        addEventToResult(event, `unknownEvent_${(0, import_Number.toHexString)(eventId)}`);
        logger.info(`Added unknown event ${(0, import_Number.toHexString)(eventId)} to cluster ${(0, import_Number.toHexString)(clusterId)}.`);
      }
    }
  }
  const commandToId = {};
  ...
  if (globalAttributeValues?.acceptedCommandList !== void 0) {
    for (const requestId of globalAttributeValues.acceptedCommandList) {
      if (commandToId[requestId] === void 0) {
        logger.info(`Ignoring unknown command ${requestId} at cluster ${(0, import_Number.toHexString)(clusterId)}`);
      }
    }
  }
  return result;
