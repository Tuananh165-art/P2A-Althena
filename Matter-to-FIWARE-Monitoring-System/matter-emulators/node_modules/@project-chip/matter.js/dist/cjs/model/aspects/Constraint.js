...
class Constraint extends import_Aspect.Aspect {
  ...
  test(value) {
    ...
    if (this.value !== void 0 || value === null) {
      return false;
    }
...
((Constraint2) => {
  function parseValue(numOrName) {
    ...
    if (Number.isNaN(value)) {
      return import_definitions.FieldValue.Reference((0, import_String.camelize)(numOrName));
    }
    if (numOrName.endsWith("%")) {
      return import_definitions.FieldValue.Percent(value);
    }
    if (numOrName.endsWith("\xB0C")) {
      return import_definitions.FieldValue.Celsius(value);
    }
    return value;
  }
  ...
  function serializeAtom(ast) {
    ...
    if (ast.value !== void 0 && ast.value !== null) {
      return `${import_definitions.FieldValue.serialize(ast.value)}`;
    } else if (ast.min !== void 0 && ast.min !== null) {
      if (ast.max === void 0 || ast.max === null) {
        return `min ${import_definitions.FieldValue.serialize(ast.min)}`;
      }
      return `${import_definitions.FieldValue.serialize(ast.min)} to ${import_definitions.FieldValue.serialize(ast.max)}`;
    } else if (ast.max !== void 0 && ast.max !== null) {
      return `max ${import_definitions.FieldValue.serialize(ast.max)}`;
    }
    return "all";
  }
