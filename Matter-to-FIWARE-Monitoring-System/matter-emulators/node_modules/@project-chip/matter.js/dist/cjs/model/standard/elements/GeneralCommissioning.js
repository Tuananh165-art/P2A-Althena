...
import_Matter.Matter.children.push({
  ...
  details: "This cluster is used to manage basic commissioning lifecycle.\nThis cluster also represents responsibilities related to commissioning that don\u2019t well fit other commissioning clusters, like Section 11.8, \u201CNetwork Commissioning Cluster\u201D. It also hosts functionalities\nthose other clusters may depend on.",
...
period, can only be invoked over a CASE session, and only if no AddNOC command was previously processed in the same fail-safe timer period.
On creation of the Fail Safe Context a second timer shall be created to expire at MaxCumulativeFailsafeSeconds as specified in BasicCommissioningInfo. This Cumulative Fail Safe Context timer (CFSC timer) serves to limit the lifetime of any particular Fail Safe Context; it shall NOT be extended or modified on subsequent invocations of ArmFailSafe associated with this Fail Safe Context. Upon expiry of the CFSC timer, the receiver shall execute cleanup behavior equivalent to that of fail-safe timer expiration as detailed in Section 11.9.6.2.2, \u201CBehavior on expiry of Fail-Safe timer\u201D. Termination of the session prior to the expiration of that timer for any reason (including a successful end of commissioning or an expiry of a fail-safe timer) shall also delete the CFSC timer.
...
If the fail-safe timer expires before the CommissioningComplete command is successfully invoked, the following sequence of clean-up steps shall be executed, in order, by the receiver:
  ...
  9. Optionally: if no factory-reset resulted from the previous steps, it is RECOMMENDED that the      Node rollback the state of all non fabric-scoped data present in the Fail-Safe context.`,
      ...
      children: [
        ...
        { tag: "datatype", name: "Breadcrumb", id: 1, type: "uint64", conformance: "M" }
      ]
    },
    {
      tag: "command",
    ...
    },
    {
      ...
      children: [
        ...
        { tag: "datatype", name: "CountryCode", id: 1, type: "string", conformance: "M", constraint: "2" },
        { tag: "datatype", name: "Breadcrumb", id: 2, type: "uint64", conformance: "M" }
      ]
    },
    {
      tag: "command",
      ...
      children: [
        ...
        { tag: "datatype", name: "DebugText", id: 1, type: "string", conformance: "M", default: "" }
      ]
    },
    {
      tag: "command",
    ...
    },
    {
      ...
      children: [
        { tag: "datatype", name: "Ok", id: 0, conformance: "M", description: "No error" },
        {
          tag: "datatype",
    ...
    },
    {
      ...
      children: [
        { tag: "datatype", name: "Indoor", id: 0, conformance: "M", description: "Indoor only" },
        { tag: "datatype", name: "Outdoor", id: 1, conformance: "M", description: "Outdoor only" },
        { tag: "datatype", name: "IndoorOutdoor", id: 2, conformance: "M", description: "Indoor/Outdoor" }
      ]
    },
    {
      tag: "datatype",
