...
var import_EventClient = require("../cluster/client/EventClient.js");
 ...
 */
const logger = import_Logger.Logger.get("EndpointStructureLogger");
function getAttributeServerValue(attribute, options = {}) {
  ...
  try {
    ...
    if ((attributeValueType !== "object" || attributeValue === null) && options.logAttributePrimitiveValues !== false) {
      ...
      value = import_Logger.Logger.toJSON(attributeValue);
    }
  } catch (error) {
...
function logClusterServer(endpoint, clusterServer, options = {}) {
  if (options.clusterServerFilter !== void 0 && !options.clusterServerFilter(endpoint, clusterServer))
    return;
  const featureMap = clusterServer.attributes.featureMap?.getLocal() ?? {};
  const globalAttributes = (0, import_Cluster.GlobalAttributes)(featureMap);
  ...
  logger.info(
    `Cluster-Server "${clusterServer.name}" (${(0, import_Number.toHexString)(clusterServer.id)}) ${supportedFeatures.length ? `(Features: ${supportedFeatures.join(", ")})` : ""}`
  );
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        for (const attributeName in globalAttributes) {
          ...
          logger.info(
            `"${attribute.name}" (${(0, import_Number.toHexString)(attribute.id)})${value !== "" ? `: value = ${value}` : ""}`
          );
        }
      });
    });
  ...
  if (options.logClusterCommands !== false) {
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        const commands = (0, import_ClusterServerTypes.asClusterServerInternal)(clusterServer)._commands;
        for (const commandName in commands) {
          const command = commands[commandName];
          if (command === void 0)
            continue;
          logger.info(`"${command.name}" (${(0, import_Number.toHexString)(command.invokeId)}/${command.responseId})`);
        }
      });
    });
  }
  if (options.logClusterEvents !== false) {
    import_Logger.Logger.nest(() => {
      logger.info("Events:");
      import_Logger.Logger.nest(() => {
        const events = (0, import_ClusterServerTypes.asClusterServerInternal)(clusterServer)._events;
        for (const eventName in events) {
          const event = events[eventName];
          if (event === void 0)
            continue;
          logger.info(`"${event.name}" (${(0, import_Number.toHexString)(event.id)})`);
        }
      });
    });
  }
...
function logClusterClient(endpoint, clusterClient, options = {}) {
  if (options.clusterClientFilter !== void 0 && !options.clusterClientFilter(endpoint, clusterClient))
    return;
  const { supportedFeatures: features } = clusterClient;
  const globalAttributes = (0, import_Cluster.GlobalAttributes)(features);
  const supportedFeatures = new Array();
  ...
  logger.info(
    `Cluster-Client "${clusterClient.name}" (${(0, import_Number.toHexString)(clusterClient.id)}) ${supportedFeatures.length ? `(Features: ${supportedFeatures.join(", ")})` : ""}`
  );
  if (options.logClusterGlobalAttributes !== false) {
    import_Logger.Logger.nest(() => {
      logger.info("Global-Attributes:");
      import_Logger.Logger.nest(() => {
        for (const attributeName in globalAttributes) {
          ...
          if (attribute === void 0)
            continue;
          logger.info(`"${attribute.name}" (${(0, import_Number.toHexString)(attribute.id)})`);
        }
      });
    });
  }
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        for (const attributeName in clusterClient.attributes) {
          ...
          if (unknown)
            info += " (Unknown)";
          logger.info(`"${attribute.name}" (${(0, import_Number.toHexString)(attribute.id)})${info}`);
        }
      });
    });
  }
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        for (const eventName in clusterClient.events) {
          ...
          if (event === void 0)
            continue;
          const supported = event instanceof import_EventClient.SupportedEventClient;
          if (!supported && options.logNotSupportedClusterEvents === false)
            continue;
          const unknown = event instanceof import_EventClient.UnknownSupportedEventClient;
          let info = "";
          if (!supported)
            info += " (Not Supported)";
          if (unknown)
            info += " (Unknown)";
          logger.info(`"${event.name}" (${(0, import_Number.toHexString)(event.id)})${info}`);
        }
      });
    });
  }
...
function logEndpoint(endpoint, options = {
  ...
  if (options.endpointFilter !== void 0 && !options.endpointFilter(endpoint))
    return;
