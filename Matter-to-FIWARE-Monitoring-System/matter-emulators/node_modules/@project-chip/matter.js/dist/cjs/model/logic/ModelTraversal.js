...
var ModelTraversal_exports = {};
__export(ModelTraversal_exports, {
  ModelTraversal: () => ModelTraversal
});
module.exports = __toCommonJS(ModelTraversal_exports);
...
class ModelTraversal {
  ...
  operation(operator, toDismiss) {
    if (this.operationDepth > OPERATION_DEPTH_LIMIT) {
      throw new import_MatterError.InternalError("Likely cycle detected (or OPERATION_DEPTH_LIMIT needs to be bumped)");
    }
    if (toDismiss) {
      if (!this.dismissed) {
  ...
  getTypeName(model) {
    return this.operation(() => {
      ...
      if (model.tag === import_definitions.ElementTag.Command || model.tag === import_definitions.ElementTag.Event) {
        return "struct";
      }
      let result;
      ...
      this.visitInheritance(model.parent, (ancestor) => {
        if (ancestor.metatype) {
          switch (ancestor.name) {
            case import_elements.Globals.enum8.name:
            case import_elements.Globals.map8.name:
              result = import_elements.Globals.uint8.name;
              return false;
            case import_elements.Globals.enum16.name:
            case import_elements.Globals.map16.name:
              result = import_elements.Globals.uint16.name;
              return false;
            case import_elements.Globals.map32.name:
              result = import_elements.Globals.uint32.name;
              return false;
            case import_elements.Globals.map64.name:
              result = import_elements.Globals.uint64.name;
              return false;
          }
        }
  ...
  findConstraint(model, symbol, field) {
    return this.operation(() => {
      ...
      const resolve = (field2) => {
        ...
        const name = import_definitions.FieldValue.referenced(value);
        if (name === void 0) {
          return;
        }
        const referenced = this.findMember(model.parent, name, [
          import_definitions.ElementTag.Attribute,
          import_definitions.ElementTag.Datatype
        ]);
        if (!referenced) {
          return;
        }
  ...
  findBitDefinition(scope, bit) {
    return this.operation(() => {
      while (scope) {
        ...
        if (scope.effectiveMetatype !== import_definitions.Metatype.bitmap) {
          scope = scope.parent;
          continue;
        }
  ...
  findResponse(command) {
    if (command.response && command.response !== "status") {
      return new ModelTraversal().findType(command, command.response, import_definitions.ElementTag.Command);
    }
  }
  /**
  ...
  findReferences(scope, type) {
    ...
    this.visit(scope, (model) => {
      ...
      if (model instanceof import_models.CommandModel && this.findResponse(model) === type) {
        references.push(model);
        return;
      }
      if (model.isType) {
        const defaultValue = model.default;
        if (import_definitions.FieldValue.is(defaultValue, import_definitions.FieldValue.reference)) {
