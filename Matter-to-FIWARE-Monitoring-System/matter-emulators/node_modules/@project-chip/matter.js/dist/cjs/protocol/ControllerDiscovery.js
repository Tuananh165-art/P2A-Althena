 ...
 */
const logger = import_Logger.Logger.get("ControllerDiscovery");
class DiscoveryError extends import_MatterError.MatterError {
}
class ControllerDiscovery {
  ...
  static async discoverDeviceAddressesByIdentifier(scanners, identifier, timeoutSeconds = 30) {
    logger.info(`Start Discovering devices using identifier ${import_Logger.Logger.toJSON(identifier)} ...`);
    const scanResults = scanners.map(async (scanner) => {
      ...
      logger.info(`Found ${foundDevices.length} devices using identifier ${import_Logger.Logger.toJSON(identifier)}`);
      if (foundDevices.length === 0) {
        throw new import_ControllerCommissioner.CommissioningError(
          `No device discovered using identifier ${import_Logger.Logger.toJSON(
            identifier
      ...
      if (devices.length === 0) {
        throw new import_ControllerCommissioner.CommissioningError(
          `Device discovered using identifier ${import_Logger.Logger.toJSON(
            identifier
    ...
    return await (0, import_Promises.anyPromise)(scanResults);
  }
  ...
  static async iterateServerAddresses(devices, errorType, updateDevicesFunc, func, lastKnownAddress) {
    const processOneAddress = async (address, device) => {
      const serverKey = (0, import_ServerAddress.serverAddressToString)(address);
    ...
    devices.forEach(
      (device) => device.addresses.forEach((address) => addresses.set((0, import_ServerAddress.serverAddressToString)(address), { address, device }))
    );
    const triedAddresses = /* @__PURE__ */ new Set();
    if (lastKnownAddress !== void 0) {
      const knownKey = (0, import_ServerAddress.serverAddressToString)(lastKnownAddress);
      const knownDevice = addresses.has((0, import_ServerAddress.serverAddressToString)(lastKnownAddress)) ? addresses.get(knownKey)?.device : void 0;
    ...
    while (true) {
      logger.debug(
        `Server addresses to try: ${Array.from(addresses).map(([addressString, { device }]) => `${device?.DN}: ${addressString}`).join(",")}`
      );
      let triedOne = false;
      for (const { address, device } of addresses.values()) {
        const serverKey = (0, import_ServerAddress.serverAddressToString)(address);
        if (triedAddresses.has(serverKey))
          continue;
      ...
      if (triedOne) {
        (await updateDevicesFunc()).forEach(
          (device) => device.addresses.forEach(
            (address) => addresses.set((0, import_ServerAddress.serverAddressToString)(address), { address, device })
          )
        );
      } else {
        throw new import_MatterController.PairRetransmissionLimitReachedError(`Failed to connect on any discovered server`);
      }
    }
  }
}
