...
class Base38Schema extends import_Schema.Schema {
  ...
  decodeInternal(encoded) {
    ...
    switch (remainderEncodedLength) {
      ...
      case 2:
        ...
        break;
      default:
        throw new import_MatterError.UnexpectedDataError(`Invalid base38 encoded string length: ${encodedLength}`);
    }
    const result = new import_ByteArray.ByteArray(decodeLength);
    let decodedOffset = 0;
    let encodedOffset = 0;
  ...
  decodeBase38(encoded, offset, charCount) {
    ...
    for (let i = charCount - 1; i >= 0; i--) {
      ...
      if (code === -1)
        throw new import_MatterError.UnexpectedDataError(`Unexpected character ${char} at ${offset + i}`);
