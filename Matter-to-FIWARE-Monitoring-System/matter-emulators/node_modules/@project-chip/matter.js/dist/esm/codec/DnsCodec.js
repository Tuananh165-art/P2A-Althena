...
class DnsCodec {
  ...
  static encode({
    ...
    if (messageType === void 0)
      throw new InternalError("Message type must be specified!");
    ...
    if (authorities.length > 0)
      throw new NotImplementedError("Authority answers are not supported yet!");
  ...
  static encodeARecord(ip) {
    if (!isIPv4(ip))
      throw new UnexpectedDataError(`Invalid A Record value: ${ip}`);
    const writer = new DataWriter(Endian.Big);
  ...
  static encodeAaaaRecord(ip) {
    if (!isIPv6(ip))
      throw new UnexpectedDataError(`Invalid AAAA Record value: ${ip}`);
    const writer = new DataWriter(Endian.Big);
