...
var import_TimeSyncCluster = require("../cluster/definitions/TimeSyncCluster.js");
...
class ControllerCommissioner {
  constructor(interactionClient, certificateManager, fabric, commissioningOptions, nodeId, adminVendorId, reconnectWithDeviceCallback) {
    ...
    this.reconnectWithDeviceCallback = reconnectWithDeviceCallback;
    this.commissioningSteps = new Array();
    this.commissioningStepResults = /* @__PURE__ */ new Map();
    this.clusterClients = /* @__PURE__ */ new Map();
    this.lastBreadcrumb = 1;
    this.collectedCommissioningData = {};
    this.failSafeTimeMs = DEFAULT_FAILSAFE_TIME_MS;
  ...
  ensureOperationalCredentialsSuccess(context, { statusCode, debugText, fabricIndex }) {
    ...
    if (statusCode === import_OperationalCredentialsCluster.OperationalCredentials.NodeOperationalCertStatus.Ok)
      return;
  ...
  async getInitialData() {
    ...
    const networkData = await this.interactionClient.getMultipleAttributes({
      attributes: [
        {
          clusterId: import_NetworkCommissioningCluster.NetworkCommissioning.Cluster.id,
          attributeId: import_NetworkCommissioningCluster.NetworkCommissioning.Cluster.attributes.featureMap.id
        },
        {
          clusterId: import_NetworkCommissioningCluster.NetworkCommissioning.Cluster.id,
          attributeId: import_NetworkCommissioningCluster.NetworkCommissioning.Cluster.attributes.networks.id
        }
      ]
    });
    ...
    for (const {
      ...
      if (attributeId === import_NetworkCommissioningCluster.NetworkCommissioning.Cluster.attributes.featureMap.id) {
        networkFeatures.push({ endpointId, value });
      } else if (attributeId === import_NetworkCommissioningCluster.NetworkCommissioning.Cluster.attributes.networks.id) {
  ...
  async synchronizeTime() {
    if (this.collectedCommissioningData.rootServerList !== void 0 && this.collectedCommissioningData.rootServerList.find((clusterId) => clusterId === import_TimeSyncCluster.TimeSyncCluster.id)) {
  ...
  async certificates() {
    ...
    const { certSigningRequest } = import_OperationalCredentialsServer.TlvCertSigningRequest.decode(nocsrElements);
  ...
  async validateNetwork() {
    ...
    if (this.commissioningOptions.wifiNetwork === void 0 && this.commissioningOptions.threadNetwork === void 0) {
      ...
      const anyInterfaceConnected = this.collectedCommissioningData.networkStatus.length === 0 || this.collectedCommissioningData.networkStatus.some(
        ({ value }) => value.some(({ connected }) => connected === true)
      );
  ...
  async configureNetworkWifi() {
    ...
    if (this.collectedCommissioningData.networkFeatures !== void 0 && this.collectedCommissioningData.networkStatus !== void 0) {
      ...
      if (rootNetworkStatus !== void 0 && rootNetworkStatus.length > 0 && rootNetworkStatus[0].connected !== false) {
