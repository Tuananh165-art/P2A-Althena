...
class ArraySchema extends import_TlvSchema.TlvSchema {
  ...
  decodeTlvInternalValue(reader, typeLength) {
    if (typeLength.type !== import_TlvCodec.TlvType.Array)
      throw new import_MatterError.UnexpectedDataError(`Unexpected type ${typeLength.type}, expected Array (${import_TlvCodec.TlvType.Array}).`);
    const result = new Array();
    while (true) {
      const { tag: elementTag, typeLength: elementTypeLength } = reader.readTagType();
      if (elementTag !== void 0)
        throw new import_MatterError.UnexpectedDataError("Array element tags should be anonymous.");
      if (elementTypeLength.type === import_TlvCodec.TlvType.EndOfContainer)
        break;
  ...
  validate(data) {
    if (!Array.isArray(data))
      throw new import_ValidationError.ValidationError(`Expected array, got ${typeof data}.`);
    if (data.length > this.maxLength)
      throw new import_ValidationError.ValidationError(`Array is too long: ${data.length}, max ${this.maxLength}.`);
    if (data.length < this.minLength)
      throw new import_ValidationError.ValidationError(`Array is too short: ${data.length}, min ${this.minLength}.`);
    data.forEach((element, index) => {
      (0, import_TryCatchHandler.tryCatch)(
        () => this.elementSchema.validate(element),
        import_ValidationError.ValidationError,
        (error) => {
          error.fieldName = `[${index}]${error.fieldName !== void 0 ? `.${error.fieldName}` : ""}`;
          throw error;
        }
      );
    });
  ...
  encodeAsChunkedArray(value) {
    const result = [];
    result.push({ listIndex: void 0, element: this.encodeTlv([]) });
