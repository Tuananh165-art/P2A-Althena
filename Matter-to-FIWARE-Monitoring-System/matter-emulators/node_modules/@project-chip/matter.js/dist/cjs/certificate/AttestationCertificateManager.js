...
class AttestationCertificateManager {
  constructor(vendorId) {
    ...
    this.paaKeyPair = (0, import_Key.PrivateKey)(import_ChipPAAuthorities.TestCert_PAA_NoVID_PrivateKey, {
      publicKey: import_ChipPAAuthorities.TestCert_PAA_NoVID_PublicKey
    });
    this.paaKeyIdentifier = import_ChipPAAuthorities.TestCert_PAA_NoVID_SKID;
    this.paiCertId = BigInt(1);
    this.paiKeyPair = import_Crypto.Crypto.createKeyPair();
    this.paiKeyIdentifier = import_Crypto.Crypto.hash(this.paiKeyPair.publicKey).slice(0, 20);
    this.nextCertificateId = 2;
  ...
  getDACert(productId) {
    const dacKeyPair = import_Crypto.Crypto.createKeyPair();
    return {
      keyPair: dacKeyPair,
  ...
  generatePAACert(vendorId) {
    ...
    const unsignedCertificate = {
      serialNumber: import_ByteArray.ByteArray.of(Number(this.paaCertId)),
    ...
    return import_CertificateManager.CertificateManager.paaCertToAsn1(unsignedCertificate, this.paaKeyPair);
  }
  generatePAICert(vendorId, productId) {
    const now = import_Time.Time.get().now();
    const unsignedCertificate = {
      serialNumber: import_ByteArray.ByteArray.of(Number(this.paiCertId)),
      signatureAlgorithm: 1,
      publicKeyAlgorithm: 1,
      ellipticCurveIdentifier: 1,
      issuer: {
    ...
    return import_CertificateManager.CertificateManager.paiCertToAsn1(unsignedCertificate, this.paaKeyPair);
  }
  generateDaCert(publicKey, vendorId, productId) {
    ...
    return import_CertificateManager.CertificateManager.daCertToAsn1(unsignedCertificate, this.paiKeyPair);
  }
}
