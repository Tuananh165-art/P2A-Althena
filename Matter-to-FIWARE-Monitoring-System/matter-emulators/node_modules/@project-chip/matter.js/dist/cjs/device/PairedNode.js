...
class PairedNode {
  constructor(nodeId, commissioningController, options = {}, reconnectInteractionClient, assignDisconnectedHandler) {
    ...
    this.reconnectDelayTimer = import_Time.Time.getTimer(
      STRUCTURE_UPDATE_TIMEOUT_MS,
      async () => await this.reconnect()
    );
    this.updateEndpointStructureTimer = import_Time.Time.getTimer(
      5e3,
  ...
  async reconnect() {
    ...
    try {
      ...
      if (error instanceof import_MatterError.MatterError) {
  ...
  async subscribeAllAttributesAndEvents(options) {
    ...
    const initialSubscriptionData = await interactionClient.subscribeAllAttributesAndEvents({
      ...
      attributeListener: (data) => {
        ...
        if (cluster === void 0) {
          logger.info(
            `Ignoring received attribute update for unknown cluster ${(0, import_Number.toHexString)(
              clusterId
        ...
        logger.debug(
          `Trigger attribute update for ${endpointId}.${cluster.name}.${attributeId} to ${import_Logger.Logger.toJSON(
            value
          )}`
        );
      ...
      eventListener: (data) => {
        ...
        if (cluster === void 0) {
          logger.info(
            `Ignoring received event for unknown cluster ${(0, import_Number.toHexString)(
              clusterId
            )} on endpoint ${endpointId}!`
          );
          return;
  ...
  async openBasicCommissioningWindow(commissioningTimeout = 900) {
    ...
    try {
      ...
      if (!(error instanceof import_StatusCode.StatusResponseError) || error.code !== import_StatusCode.StatusCode.Failure || error.clusterCode !== import_definitions.AdministratorCommissioning.StatusCode.WindowNotOpen) {
        throw error;
      }
    }
  ...
  async openEnhancedCommissioningWindow(commissioningTimeout = 900) {
    ...
    const qrPairingCode = import_PairingCodeSchema.QrPairingCodeCodec.encode({
      ...
      vendorId,
      productId,
      flowType: import_PairingCodeSchema.CommissionningFlowType.Standard,
      discriminator,
      passcode,
