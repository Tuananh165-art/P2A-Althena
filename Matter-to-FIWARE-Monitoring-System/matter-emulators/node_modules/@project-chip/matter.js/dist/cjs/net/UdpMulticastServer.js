...
class UdpMulticastServer {
  constructor(network, broadcastAddressIpv4, broadcastAddressIpv6, broadcastPort, serverIpv4, serverIpv6, netInterface) {
    ...
    this.broadcastChannels = new import_Cache.Cache(
  ...
  static async create({
    ...
    const network = import_Network.Network.get();
  ...
  async send(message, netInterface, uniCastTarget) {
    ...
    if (uniCastTarget !== void 0 && netInterface !== void 0) {
      ...
      const netInterfaces = netInterface !== void 0 ? [netInterface] : this.network.getNetInterfaces();
      await Promise.all(
        netInterfaces.map(async (netInterface2) => {
          const { ips } = this.network.getIpMac(netInterface2) ?? { ips: [] };
          await Promise.all(
            ips.map(async (ip) => {
              const iPv4 = (0, import_Ip.isIPv4)(ip);
