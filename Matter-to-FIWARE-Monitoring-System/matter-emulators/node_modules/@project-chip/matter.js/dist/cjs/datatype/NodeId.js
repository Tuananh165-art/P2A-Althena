"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var NodeId_exports = {};
__export(NodeId_exports, {
  NodeId: () => NodeId,
  TlvNodeId: () => TlvNodeId
});
module.exports = __toCommonJS(NodeId_exports);
var import_MatterError = require("../common/MatterError.js");
var import_Crypto = require("../crypto/Crypto.js");
var import_TlvNumber = require("../tlv/TlvNumber.js");
var import_TlvWrapper = require("../tlv/TlvWrapper.js");
var import_ByteArray = require("../util/ByteArray.js");
var import_DataWriter = require("../util/DataWriter.js");
var import_Number = require("../util/Number.js");
var import_CaseAuthenticatedTag = require("./CaseAuthenticatedTag.js");
/**
 * @license
 * Copyright 2022-2023 Project CHIP Authors
 * SPDX-License-Identifier: Apache-2.0
 */
function NodeId(v) {
  return (0, import_Number.toBigInt)(v);
}
((NodeId2) => {
  const OPERATIONAL_NODE_MIN = BigInt("0x0000000000000001");
  const OPERATIONAL_NODE_MAX = BigInt("0xFFFFFFEFFFFFFFFF");
  NodeId2.UNSPECIFIED_NODE_ID = NodeId2(0);
  NodeId2.toHexString = (nodeId) => {
    const writer = new import_DataWriter.DataWriter(import_ByteArray.Endian.Big);
    writer.writeUInt64(nodeId);
    return writer.toByteArray().toHex().toUpperCase();
  };
  NodeId2.getRandomOperationalNodeId = () => {
    while (true) {
      const randomBigInt = BigInt("0x" + import_Crypto.Crypto.getRandomData(8).toHex());
      if (randomBigInt >= OPERATIONAL_NODE_MIN && randomBigInt <= OPERATIONAL_NODE_MAX) {
        return NodeId2(randomBigInt);
      }
    }
  };
  NodeId2.getFromGroupNodeId = (groupId) => {
    if (groupId < 0 || groupId > 65535) {
      throw new import_MatterError.UnexpectedDataError(`Invalid group ID: ${groupId}`);
    }
    return NodeId2(BigInt("0xFFFFFFFFFFFF" + groupId.toString(16).padStart(4, "0")));
  };
  NodeId2.getFromTemporaryLocalNodeId = (id) => {
    if (id < 0 || id > 4294967295) {
      throw new import_MatterError.UnexpectedDataError(`Invalid ID: ${id}`);
    }
    return NodeId2(BigInt("0xFFFFFFFE" + id.toString(16).padStart(8, "0")));
  };
  NodeId2.getFromCaseAuthenticatedTag = (id) => {
    if (id < 0 || id > 4294967295) {
      throw new import_MatterError.UnexpectedDataError(`Invalid CASE Authenticated tag: ${id}`);
    }
    return NodeId2(BigInt("0xFFFFFFFD" + id.toString(16).padStart(8, "0")));
  };
  NodeId2.extractAsCaseAuthenticatedTag = (nodeId) => {
    return (0, import_CaseAuthenticatedTag.CaseAuthenticatedTag)(Number(nodeId.toString(16).slice(8)));
  };
  NodeId2.getFromPakeKeyIdentifier = (id) => {
    if (id < 0 || id > 4294967295) {
      throw new import_MatterError.UnexpectedDataError(`Invalid ID: ${id}`);
    }
    return NodeId2(BigInt("0xFFFFFFFB" + id.toString(16).padStart(8, "0")));
  };
})(NodeId || (NodeId = {}));
const TlvNodeId = new import_TlvWrapper.TlvWrapper(
  import_TlvNumber.TlvUInt64,
  (nodeId) => nodeId,
  (value) => NodeId(BigInt(value))
);
//# sourceMappingURL=NodeId.js.map
