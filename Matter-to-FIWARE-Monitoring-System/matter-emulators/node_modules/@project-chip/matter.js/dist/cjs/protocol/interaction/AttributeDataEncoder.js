...
function encodeAttributePayload(attributePayload) {
  ...
  if (attributeData === void 0) {
    return import_InteractionProtocol.TlvAttributeReport.encodeTlv({ attributeStatus });
  }
  const { path, schema, payload, dataVersion } = attributeData;
  return import_InteractionProtocol.TlvAttributeReport.encodeTlv({ attributeData: { path, data: schema.encodeTlv(payload), dataVersion } });
}
function encodeEventPayload(eventPayload) {
  const { eventData, eventStatus } = eventPayload;
  if (eventData === void 0) {
    return import_InteractionProtocol.TlvEventReport.encodeTlv({ eventStatus });
  }
  const {
    path,
    schema,
  ...
  return import_InteractionProtocol.TlvEventReport.encodeTlv({
    eventData: {
      path,
...
function canAttributePayloadBeChunked(attributePayload) {
  ...
  return schema instanceof import_TlvArray.ArraySchema && Array.isArray(payload) && payload.length > 0 && listIndex === void 0;
}
function chunkAttributePayload(attributePayload) {
  const { attributeData } = attributePayload;
  if (attributeData === void 0) {
    throw new import_MatterError.MatterFlowError(
      `Can not chunk an AttributePayload with just a attributeStatus: ${import_Logger.Logger.toJSON(attributePayload)}`
    );
  }
  const { schema, path, dataVersion, payload } = attributeData;
  if (!(schema instanceof import_TlvArray.ArraySchema) || !Array.isArray(payload)) {
    throw new import_MatterError.MatterFlowError(
      `Can not chunk an AttributePayload with attributeData that is not an array: ${import_Logger.Logger.toJSON(
        attributePayload
      )}`
    );
  }
  const chunks = new Array();
  chunks.push({ attributeData: { schema, path: { ...path, listIndex: void 0 }, payload: [], dataVersion } });
