...
__export(Logger_exports, {
  DiagnosticDictionary: () => DiagnosticDictionary,
  Format: () => Format,
  Level: () => Level,
  Logger: () => Logger,
...
function determineLoggerForFormat(format) {
  switch (format) {
    ...
    default:
      throw new import_MatterError.NotImplementedError(`Unsupported log format "${format}"`);
  }
}
class Logger {
  constructor(name) {
  ...
  log(level, values) {
    Logger.logger.forEach((logger) => {
      if (level < (logger.logLevels[this.name] ?? logger.defaultLogLevel))
        return;
      logger.log(level, logger.logFormatter(import_Time.Time.now(), level, this.name, values));
    });
  }
}
